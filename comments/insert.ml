      ...
      (match first.T.desc with
       ...
       | Comment c when not !(c.explicitely_inserted) ->
         let acc =
           ...
           (* It looks like we might reorder comment if some [Floating] comments follow
              some [After] ones. But that cannot happen, by construction.
           *)
           | After -> floating, Doc.Utils.(attached_after ^?^ cmt)
           | Floating -> Doc.Utils.(floating ^?^ cmt), attached_after
           (* Ideally we'd [assert false] here: [Before] comments have necessarily been
              consumed already.

              ... however, comments at the beginning of the file (before anycode)
              currently get marked as [Before] by the lexer. This is should be fixed
              eventually (because some might actually be [After]!) but for now we treat
              them as floating here. *)
           | Before -> Doc.Utils.(floating ^?^ cmt), attached_after
         in
         aux acc rest
       | Comment _ | Token _ -> acc, first :: rest)
  in
  aux Doc.(empty, empty)
;;
...
type state =
  { space_handling : special_space_treatement
       (** Whether a space should be inserted before the next leaf node (token or comment).

      This flag is set when a comment is inserted, and it is reset when a space is
      inserted (either manually, or because we encountered a [Whitespace] node). *)
...
let saw_leaf st = { st with space_handling = Insert_before_inserting_comment }
...
let attach_before_comments state tokens doc =
  ...
  then (* delay until outside the group. *) tokens, doc, state
    ...
    | [] -> (* no comment to attach *) tokens, doc, state
    | to_append ->
      let tokens = Base.List.drop_while ~f:is_comment_attaching_before tokens in
      let doc =
        let open Doc in
        group
        @@ List.fold_left
...
let insert_space_if_required ?(inserting_comment = false) state doc =
  let brk =
    ...
    | Insert_before_leaf, _ | Insert_before_inserting_comment, true ->
      Doc.break 1
    | _ -> Doc.empty
  in
  Doc.(brk ^^ doc)
...
(** Traverse the document and sequence of tokens simultaneously, extending the document's
    structure with a subdocument for any comment that might be missing.

    We take some care to make these insertions fit in a nice way in the whole document:
    - we add some spacing so they aren't just spliced before/after the token they attach
      to, while being careful to not add spaces where there are already some
    - we try to insert the comments outside of [Doc.Group]s, as this often negatively
      impacts the layout of the code.

    However, we also try to respect comment's "attachement" (cf. [Tokens.attachement]),
    which roughly means having them on the same line or at the same indentation level as
    the token they attach to. This sometimes implies that we will insert comments inside a
    group, so we can reach the correct indentation/nesting level. Refer to the lexer the
    actual rules regarding attachement. *)
  ...
  | first :: rest ->
    (match first.T.desc, doc with
     ...
     | T.Comment { explicitely_inserted; _ }, Doc.Comment _
       when !explicitely_inserted -> rest, doc, state
     | _, Doc.Comment _ -> seq, doc, state
     | T.Comment { explicitely_inserted; _ }, Doc.Token _
       when !explicitely_inserted -> walk_both state rest doc
     ...
     | T.Comment { explicitely_inserted; _ }, Doc.Group (_, _, _, d)
       ...
       (* we can insert comments outside the group as they'll be at the same nesting level
          as the next word and there's no hint that comments should be inside the group. *)
...
and traverse_group tokens state margin flatness grouped_doc =
  ...
  in
  let doc =
    (* Inserting now rather than in the group so as to not break it, but... group might
       start with a space. *)
    if first_is_space d
...
let from_tokens tokens doc =
  walk_both init_state tokens doc |> append_trailing_comments
;;
