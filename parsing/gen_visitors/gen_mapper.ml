let files = ref []
let () = Arg.parse [] (fun fn -> files := fn :: !files) "gen_mapper.exe CM[OI]S"
...
let mapper_field_of_td fmt (modname, tname) td =
  ...
  let pp_vars_and_mappers fmt = function
    ...
    | vars ->
      ...
      fprintf
        fmt
        "@ %a.@ 'mapper_env mapper@ ->@ %a@ ->"
        (pp_print_list ~pp_sep:pp_print_space pp_var)
        vars
        (pp_print_list ~pp_sep:(fun fmt () -> fprintf fmt "@ ->@ ") mapper)
        vars
  in
  fprintf
    fmt
...
let mapper_call curr_unit fmt p =
  ...
  in
  let without_mapper = [ "Lexing", "position"; "Parser_tokens", "token" ] in
  ...
  | Unknown ((punit, pname) as id) ->
    if List.mem id without_mapper
    then pp_print_string fmt "id_map"
    else (
...
let rec ty_constr_mapper curr_unit fmt (path, ct_list) =
  let arg_mapper fmt ct =
    ...
    | Ttyp_tuple _ ->
      ...
      fprintf fmt ")@]"
    | Ttyp_arrow _ -> fprintf fmt "id_map"
    | _ ->
      eprintf
  ...
  let pp_args fmt = function
    | [] -> ()
    | args ->
      fprintf fmt "@ %a" (pp_print_list ~pp_sep:pp_print_space arg_mapper) args
  in
  fprintf fmt "@[<hov 2>%a%a@]" (mapper_call curr_unit) path pp_args ct_list
...
and map_field curr_unit fmt ct value_name =
  ...
  | Ttyp_arrow _ -> fprintf fmt "id_map@ env %s" value_name
  | Ttyp_var s -> fprintf fmt "map_'%s@ env %s" s value_name
  | Ttyp_tuple types -> destruct_and_map_tuple curr_unit fmt types value_name
  | Ttyp_constr (path, _, ct_list) ->
    fprintf
      fmt
      "@[<hov 2>%a@ env %s@]"
      (ty_constr_mapper curr_unit)
      (path, ct_list)
      value_name
...
let record_mapper_body ?(constr = "") curr_unit fmt rec_name lbls =
  ...
  fprintf
    ...
    (if List.mem curr_unit opened then "" else sprintf "%s." curr_unit)
    (pp_print_list ~pp_sep:(fun fmt () -> fprintf fmt ";@ ") (fun fmt ld ->
       pp_print_string fmt ld.ld_name.txt))
    lbls
;;
...
let mapper_of_variant curr_unit fmt value_name cds =
  ...
  List.iter
    (fprintf fmt "@;@[<hov 2>| %a@]" (map_constructor_decl curr_unit))
    cds
;;
...
let mapper_of_alias curr_unit fmt name ct =
  ...
  | Ttyp_constr (path, _, args) ->
    fprintf
      ...
      (path, args)
      name
  | Ttyp_tuple types -> destruct_and_map_tuple curr_unit fmt types name
  | _ ->
...
let mapper_of_td fmt ((decl_unit, ty_name) as id) td =
  ...
  fprintf
    ...
    (pp_print_list (fun fmt -> fprintf fmt "@ map_'%s"))
    vars
    ty_name;
  (match td.typ_kind with
   | Ttype_record lbls -> mapper_of_record decl_unit fmt ty_name lbls
   | Ttype_variant cds -> mapper_of_variant decl_unit fmt ty_name cds
   | Ttype_abstract ->
     mapper_of_alias decl_unit fmt ty_name (Option.get td.typ_manifest)
   | _ -> ());
  fprintf fmt "@;@]@\n"
;;
...
let binding_of_td fmt (unit_name, type_name) _ =
  ...
  fprintf fmt "@[<hov 2>@ %s =@ map_%s@]@;;" mapper_name mapper_name
;;
