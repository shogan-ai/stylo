...
let desc_as_string = function
  | Token (t, false) -> if Dbg_print.dbg then Raw.to_string t else "tok"
  | Token (t, true) ->
    if Dbg_print.dbg then "optional(" ^ Raw.to_string t ^ ")" else "opttok"
...
module Indexed_list = struct
  ...
  let consume t start stop =
    ...
    if start = stop
    then (
      (* empty rule / epsilon reduction We will return an empty list of tokens, but we
         must remember to insert a [Child_node] entry at that position, because the parent
         node in the token tree will expect a child, even if it's empty. *)
