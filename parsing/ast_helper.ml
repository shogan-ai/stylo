...
let empty_ext_attr = { pea_ext = None; pea_attrs = [] }

module Docs = struct
  let text =
    List.filter_map (fun ds ->
      simplify_ds (Some ds) |> Option.map Docstrings.docstring_body)
  ;;
...
module Typ = struct
  ...
  (* let arrow ?loc ?attrs ~tokens a b c d e = mk ?loc ?attrs ~tokens (Ptyp_arrow (a, b,
     c, d, e)) *)
  ...
  (* let package ?loc ?attrs ~tokens a b = let pkg =
     [{ ppt_ext_attr = None; ppt_name = a; ppt_eqs = b }] in mk ?loc ?attrs ~tokens
     (Ptyp_package pkg)
  *)
...
module Exp = struct
  ...
  let case ~tokens lhs ?guard rhs =
    { pc_lhs = lhs; pc_guard = guard; pc_rhs = rhs; pc_tokens = tokens }
  ;;
...
module Mty = struct
  ...
  let signature ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pmty_signature a)
  ;;
...
module Sig = struct
  ...
  (* let value ?loc a = mk ?loc (Psig_value a) let type_ ?loc rec_flag a = mk ?loc
     (Psig_type (rec_flag, a)) let type_subst ?loc a = mk ?loc (Psig_typesubst a) let
     type_extension ?loc a = mk ?loc (Psig_typext a) let exception_ ?loc a = mk ?loc
     (Psig_exception a) let module_ ?loc a = mk ?loc (Psig_module a) let mod_subst ?loc a
     = mk ?loc (Psig_modsubst a) let rec_module ?loc a = mk ?loc (Psig_recmodule a) let
     modtype ?loc a = mk ?loc (Psig_modtype a) let modtype_subst ?loc a = mk ?loc
     (Psig_modtypesubst a) let open_ ?loc a = mk ?loc (Psig_open a) let include_ ?loc
     ?(modalities = []) a = mk ?loc (Psig_include (a, modalities)) let class_ ?loc a = mk
     ?loc (Psig_class a) let class_type ?loc a = mk ?loc (Psig_class_type a) let extension
     ?loc ?(attrs = []) a = mk ?loc (Psig_extension (a, attrs)) let kind_abbrev ?loc a b =
     mk ?loc (Psig_kind_abbrev (a, b)) let attribute ?loc ~tokens a = mk ?loc ~tokens
     (Psig_attribute a)
  *)
  let text txt =
...
module Str = struct
  ...
  (* let value ?loc a b = mk ?loc (Pstr_value (a, b)) let primitive ?loc a = mk ?loc
     (Pstr_primitive a) let type_ ?loc rec_flag a = mk ?loc (Pstr_type (rec_flag, a)) let
     type_extension ?loc a = mk ?loc (Pstr_typext a) let exception_ ?loc a = mk ?loc
     (Pstr_exception a) let module_ ?loc a = mk ?loc (Pstr_module a) let rec_module ?loc a
     = mk ?loc (Pstr_recmodule a) let modtype ?loc a = mk ?loc (Pstr_modtype a) let open_
     ?loc a = mk ?loc (Pstr_open a) let class_ ?loc a = mk ?loc (Pstr_class a) let
     class_type ?loc a = mk ?loc (Pstr_class_type a) let include_ ?loc a = mk ?loc
     (Pstr_include a) let extension ?loc ?(attrs = []) a = mk ?loc (Pstr_extension (a,
     attrs)) let kind_abbrev ?loc a b = mk ?loc (Pstr_kind_abbrev (a, b)) let attribute
     ?loc ~tokens a = mk ?loc ~tokens (Pstr_attribute a)
  *)
  let text txt =
    let f_txt = List.filter (fun ds -> docstring_body ds <> "") txt in
    List.map
...
module Cty = struct
  ...
  let signature ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pcty_signature a)
  ;;
...
module Ctf = struct
  let mk ?(loc = !default_loc) ?(attrs = []) ~tokens ?(docs = empty_docs) d =
  ...
  (* let inherit_ ?loc ?attrs a = mk ?loc ?attrs (Pctf_inherit a) let val_ ?loc ?attrs a b
     c d = mk ?loc ?attrs (Pctf_val (a, b, c, d)) let method_ ?loc ?attrs a b c d = mk
     ?loc ?attrs (Pctf_method (a, b, c, d)) let constraint_ ?loc ?attrs a b = mk ?loc
     ?attrs (Pctf_constraint (a, b)) let extension ?loc ?attrs a = mk ?loc ?attrs
     (Pctf_extension a) let attribute ?loc a = mk ?loc (Pctf_attribute a)
  *)
  let text txt =
    let f_txt = List.filter (fun ds -> docstring_body ds <> "") txt in
    List.map
...
module Cf = struct
  ...
  (* let inherit_ ?loc ?attrs a b c = mk ?loc ?attrs (Pcf_inherit (a, b, c)) let val_ ?loc
     ?attrs a b c = mk ?loc ?attrs (Pcf_val (a, b, c)) let method_ ?loc ?attrs a b c = mk
     ?loc ?attrs (Pcf_method (a, b, c)) let constraint_ ?loc ?attrs a b = mk ?loc ?attrs
     (Pcf_constraint (a, b)) let initializer_ ?loc ?attrs a = mk ?loc ?attrs
     (Pcf_initializer a) let extension ?loc ?attrs a = mk ?loc ?attrs (Pcf_extension a)
