...
(* We pass [--strategy simplified] to Menhir, which means that we wish to use its
   "simplified" strategy for handling errors. When a syntax error occurs, the current
   token is replaced with an [error] token. The parser then continues shifting and
   reducing, as far as possible. After (possibly) shifting the [error] token, though, the
   parser remains in error-handling mode, and does not request the next token, so the
   current token remains [error].

   In OCaml's grammar, the [error] token always appears at the end of a production, and
   this production always raises an exception. In such a situation, the strategy described
   above means that:

   - either the parser will not be able to shift [error], and will raise [Parser.Error];

   - or it will be able to shift [error] and will then reduce a production whose semantic
     action raises an exception.

   In either case, the parser will not attempt to read one token past the syntax error. *)
...
let prepare_error err =
  ...
  | Expecting (loc, nonterm) ->
    Location.errorf ~loc "Syntax error: %a expected." Style.inline_code nonterm
  | Not_expecting (loc, nonterm) ->
    Location.errorf
      ~loc
      "Syntax error: %a not expected."
      Style.inline_code
      nonterm
  | Applicative_path loc ->
    Location.errorf
      ~loc
      "Syntax error: applicative paths of the form %a are not supported when \
       the option %a is set."
      Style.inline_code
      "F(X).t"
      Style.inline_code
  ...
  | Variable_in_scope (loc, var) ->
    Location.errorf
      ...
      (Style.as_inline_code Pprintast.tyvar)
      var
      Style.inline_code
      var
  ...
  | Invalid_package_type (loc, ipt) ->
    let invalid ppf ipt =
      ...
      | Private_types -> Format.fprintf ppf "private types are not supported"
      | Not_with_type ->
        Format.fprintf
          ppf
          "only %a constraints are supported"
          Style.inline_code
          "with type t ="
      | Neither_identifier_nor_with_type ->
        Format.fprintf
          ...
          Style.inline_code
          "with type"
      | Misplaced_attribute -> Format.fprintf ppf "an attribute cannot go here"
    in
    Location.errorf ~loc "invalid package type: %a" invalid ipt
  | Removed_string_set loc ->
    Location.errorf
      ~loc
      "Syntax error: strings are immutable, there is no assignment syntax for \
       them.\n\
       @{<hint>Hint@}: Mutable sequences of bytes are available in the Bytes \
       module.\n\
  ...
  | Malformed_instance_identifier loc ->
    Location.errorf ~loc "Syntax error: Unexpected in module instance"
  | Unspliceable loc ->
    Location.errorf
      ~loc
      "Syntax error: expression cannot be spliced.\n\
       @{<hint>Hint@}: consider putting parentheses around the expression."
  | Let_mutable_not_allowed_at_structure_level loc ->
    Location.errorf
      ~loc
      "Syntax error: Mutable let bindings are not allowed at the structure \
       level."
  | Let_mutable_not_allowed_in_class_definition loc ->
    Location.errorf
      ~loc
      "Syntax error: Mutable let bindings are not allowed inside class \
       definitions."
  | Let_mutable_not_allowed_with_function_bindings loc ->
    Location.errorf
      ~loc
      "Syntax error: Mutable let is not allowed with function bindings.\n\
       @{<hint>Hint@}: If you really want a mutable function variable, use the \
       de-sugared syntax:\n\
      \  %a"
      Style.inline_code
      "let mutable f = fun x -> .."
