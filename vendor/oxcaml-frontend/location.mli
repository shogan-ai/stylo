...
(* This is used for reporting errors coming from the toplevel.
   ...
   [!input_phrase_buffer] should be [Some buf] where [buf] contains the last toplevel
   phrase. *)
...
(** [rewrite_absolute_path path] rewrites [path] to honor the BUILD_PATH_PREFIX_MAP
    variable if it is set. It does not check whether [path] is absolute or not. The result
    is as follows:
    - If BUILD_PATH_PREFIX_MAP is not set, just return [path].
    - otherwise, rewrite using the mapping (and if there are no matching prefixes that
      will just return [path]).

    See
    {{:https://reproducible-builds.org/specs/build-path-prefix-map/} the
      BUILD_PATH_PREFIX_MAP spec} *)
val rewrite_absolute_path : string -> string

(** [rewrite_find_first_existing path] uses a BUILD_PATH_PREFIX_MAP mapping and tries to
    find a source in mapping that maps to a result that exists in the file system. There
    ...
    - [Some target], means [target] exists and either
      ...
      - [target] is the first file (in priority order) that [path] mapped to that exists
        in the file system.
    - [Not_found] raised, means some source prefixes in the map were found that matched
      [path], but none of them existed in the file system. The caller should catch this
...
(** [rewrite_find_all_existing_dirs dir] accumulates a list of existing directories,
    [dirs], that are the result of mapping a potentially abstract directory, [dir], over
    all the mapping pairs in the BUILD_PATH_PREFIX_MAP environment variable, if any. The
    list [dirs] will be in priority order (head as highest priority).
    ...
    - Not_found raised, means some source prefixes in the map were found that matched
      [dir], but none of mapping results were existing directories (possibly due to
      misconfiguration). The caller should catch this and issue an appropriate error
      message.
...
(** [absolute_path path] first makes an absolute path, [s] from [path], prepending the
    current working directory if [path] was relative. Then [s] is rewritten using
    [rewrite_absolute_path]. Finally the result is normalized by eliminating instances of
    ['.'] or ['..']. *)
...
(** Hook for redefining the printer of reports.
    ...
    The hook is a [unit -> report_printer] and not simply a [report_printer]: this is
    useful so that it can detect the type of the output (a file, a terminal, ...) and
    select a printer accordingly. *)
...
(** [report_warning loc w] produces a report for the given warning [w], or [None] if the
    warning is not to be printed. *)
...
(** [report_alert loc w] produces a report for the given alert [w], or [None] if the alert
...
(** Raising [Error e] signals an error [e]; the exception will be caught and the error
    will be printed. *)
exception
Error
...
(** XXX uniqueness: remove this once overwriting is fully implemented *)
