...
type t =
  ...
  | Unused_var of
      ...
      (* 26 [mutated] is set if the variable was mutated ([x <- 5]), allowing for a more
  ...
  | Eol_in_string
    (* 29 Note: since OCaml 5.2, the lexer normalizes \r\n sequences in the source file to
       a single \n character, so the behavior of newlines in string literals is portable.
       This warning is never emitted anymore. *)
  ...
  | Name_out_of_scope of
      string
      *
      name_out_of_scope_warning
      (* 40 Tuple of (the type name, the name/fields out of scope) *)
...
(** Disable/enable alerts based on the parameter to the -alert command-line option. Raises
    [Arg.Bad] if the string is not a valid specification. *)
...
(** Like [Lazy.of_fun], but the function is applied with the warning/alert settings at the
    time [mk_lazy] is called. *)
