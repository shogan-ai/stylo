...
type upstream_compat_warning =
  | Immediate_erasure of
      string
      (* example: annotation in [type ('a : immediate) t = int] can't be erased. *)
  | Non_value_sort of
      string (* example: abstract type [t : float64] is marked as unboxed. *)
  | Unboxed_attribute of
      string
      (* example: unboxed attribute on an external declaration with float# is missing. *)
  | Immediate_void_variant
    (* example: [type t = A of void] is immediate, but not after erasure, which boxes
       void, so it can't be erased. *)
  | Separability_check
    (* example: [type packed = | Mk of 'a t [@@unboxed]] where ['a t : value mod non_float]. *)
...
type t =
  ...
  | Duplicate_definitions of
      ...
      *
      string (* 30 *)
      (* [Module_linked_twice of string * string * string] (* 31 *) was turned into a hard
     error *)
  ...
  | Mod_by_top of string (* 211 *) (* 212 taken *)
  | Modal_axis_specified_twice of
      { axis : string
      ; overriden_by : string
      } (* 213 *)
  | Atomic_float_record_boxed (* 214 *)
  | Implied_attribute of
      { implying : string
      ; implied : string
      } (* 215 *)

(* If you remove a warning, leave a hole in the numbering. NEVER change the numbers of
   existing warnings. If you add a new warning, add it at the end with a new number; do
   NOT reuse one of the holes.
*)
...
(* DO NOT REMOVE the ;; above: it is used by the testsuite/ests/warnings/mnemonics.mll
   test to determine where the definition of the number function above ends *)
...
type description =
  ...
  (* The compiler version introducing this warning; only tagged for warnings created after
     3.12, which introduced the numbered syntax. *)
  }
...
let descriptions =
  ...
  ; { number = 11
    ...
    ; description = "Redundant case in a pattern matching (unused match case)."
    ; since = None
    }
  ; { number = 12
  ...
  ; { number = 26
    ...
    ; description =
        ...
        \    with \"let\" or \"as\", and doesn't start with an underscore \
         (\"_\")\n\
        \    character."
    ; since = None
    }
  ...
  ; { number = 64
    ...
    ; description = "-unsafe used with a preprocessor returning a syntax tree."
    ; since = since 4 8
    }
  ; { number = 65
  ...
  ; { number = 73
    ...
    ; description =
        "A generative functor is applied to an empty structure (struct end) \
         rather than to ()."
    ; since = since 5 1
    }
  ; { number = 186
    ...
    ; description =
        ...
        \    doesn't start with an underscore (\"_\") character was never \
         mutated."
    ; since = since 5 2
    }
  ; { number = 187
  ...
  ; { number = 189
    ...
    ; description =
        "A tuple pattern ends in .. but fully matches its expected type."
    ; since = since 5 1
    }
  ; { number = 190
  ...
  ; { number = 198
    ...
    ; description =
        "A declaration whose type is an alias of a function type will be \
         ignored by zero_alloc all or all_opt."
    ; since = since 4 14
    }
  ; { number = 199
    ; names = [ "unchecked-zero-alloc-attribute" ]
    ; description =
        "A property of a function that was optimized away cannot be checked."
    ; since = since 4 14
    }
  ; { number = 210
    ; names = [ "unboxing-impossible" ]
    ; description =
        "The parameter or return value corresponding @unboxed attribute cannot \
         be unboxed."
    ; since = since 4 14
    }
  ; { number = 211
    ; names = [ "mod-by-top" ]
    ; description =
        "Including the top-most element of an axis in a kind's modifiers is a \
         no-op."
    ; since = since 4 14
    }
  ; { number = 214
    ...
    ; description =
        "Record contains atomic float fields, preventing the flat\n\
...
let letter for_debug = function
  ...
  | 'z' -> [ 27 ]
  | chr ->
    let ocamlparam_from_env =
      match Sys.getenv_opt "OCAMLPARAM" with
      | None -> "-"
      | Some value -> value
    in
    Misc.fatal_errorf
      "Warnings.letter %C (for_debug=%S, ocamlparam_from_env=%S \
       ocamlparam_from_compenv=%S)"
      chr
      for_debug
      ocamlparam_from_env
      !parsed_ocamlparam
...
let parse_alert_option s =
  ...
  let id_char = function
    ...
    | _ -> false
  in
  let rec parse_id i = if i < n && id_char s.[i] then parse_id (i + 1) else i in
  let rec scan i =
    if i = n
    then ()
    else if i + 1 = n
...
let letter_alert tokens =
  let print_warning_char ppf c =
    ...
    Format.fprintf ppf "%c%c" (if lowercase then '-' else '+') c
  in
  ...
  let consecutive_letters =
    (* we are tracking sequences of 2 or more consecutive unsigned letters in warning
       strings, for instance in '-w "not-principa"'. *)
  ...
  | example :: _ ->
    ...
    let spelling_hint ppf =
      let max_seq_len =
        List.fold_left
          (fun l x -> Misc.Stdlib.Int.max l (List.length x))
          0
          consecutive_letters
      in
      if max_seq_len >= 5
      then
        Format.fprintf
          ppf
          "@ @[Hint: Did you make a spelling mistake when using a mnemonic \
           name?@]"
      else ()
    in
    let message =
      Format.asprintf
        "@[<v>@[Setting a warning with a sequence of lowercase or uppercase \
         letters,@ like '%a',@ is deprecated.@]@ @[Use the equivalent signed \
         form:@ %t.@]@ @[Hint: Enabling or disabling a warning by its mnemonic \
         name requires a + or - prefix.@]%t@?@]"
        Format.(pp_print_list ~pp_sep:(fun _ -> ignore) pp_print_char)
        example
        (fun ppf -> List.iter (print_token ppf) tokens)
        spelling_hint
    in
    Some
      { kind = "ocaml_deprecated_cli"; use = nowhere; def = nowhere; message }
;;
...
let parse_warnings s =
  ...
  let get_range i =
    ...
    if i + 2 < String.length s && s.[i] = '.' && s.[i + 1] = '.'
    then (
  ...
  let rec loop tokens i =
    ...
    then List.rev tokens
      ...
      match s.[i] with
      | 'A' .. 'Z' | 'a' .. 'z' -> loop (Letter (s.[i], None) :: tokens) (i + 1)
...
let parse_opt error active errflag s =
  ...
  let eval = function
    ...
    | Num (n1, n2, modifier) ->
      for n = n1 to Misc.Stdlib.Int.min n2 last_warning_number do
        action modifier n
      done
  in
...
let message = function
  ...
  | Missing_record_field_pattern { form; unbound } ->
    "the following labels are not bound in this "
    ^ form
    ^ " pattern:\n"
    ^ unbound
  ...
  | Useless_record_with s ->
    "all the fields are explicitly listed in this "
    ^ s
  ...
  | No_cmx_file { missing_extension; module_name } ->
    Printf.sprintf
      "no %s file was found in path for module %s, and its interface was not \
  ...
  | Unused_field { form; field; complaint = Not_mutated } ->
    "mutable " ^ form ^ " field " ^ field ^ " is never mutated."
  ...
  | Probe_name_too_long name ->
    Printf.sprintf
      "This probe name is too long: `%s'. Probe names must be at most 100 \
       characters long."
      name
  ...
  | Unchecked_zero_alloc_attribute ->
    Printf.sprintf
      "the zero_alloc attribute cannot be checked.\n\
  ...
  | Modal_axis_specified_twice { axis; overriden_by } ->
    Printf.sprintf "This %s is overriden by %s later." axis overriden_by
  ...
  | true ->
    ...
    *)
    let sub_locs =
      ...
      then [ alert.def, "Definition"; alert.use, "Expected signature" ]
      else []
    in
  ...
  List.iter
    (fun { number; description; names; since } ->
      ...
      Printf.printf
        ...
        name
        description
        (fun out -> Option.iter (pp_since out))
        since)
    descriptions;
