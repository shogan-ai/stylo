...
module Maturity : sig
  (* Maturities are ordered such that the most experimental (Alpha) is greatest *)
  val max : maturity -> maturity -> maturity
end

(** The type of language extensions. An ['a t] is an extension that can either be off or
    be set to have any value in ['a], so a [unit t] can be either on or off, while a
    [maturity t] can have different maturity settings. *)
...
(** Require that an extension is enabled for at least the provided level, or else throw an
    exception at the provided location saying otherwise. *)
...
module Exist :
  sig
    type 'a extn =
      'a
        t
        (* this is removed from the sig by the [with] below; ocamldoc doesn't like [:=] in
       sigs *)
    ...
    (** Returns a list of all strings, like ["layouts_beta"], that correspond to this
        extension. *)
...
(** The type of language extension universes. Each universe allows a set of extensions,
    and every successive universe includes the previous one.

    Each variant corresponds to the [-extension-universe <variant>] CLI flag.

    Each extension universe, except for [No_extensions], should also have a corresponding
    library in [otherlibs/]. Those libraries must contain OCaml code for corresponding
    extensions that would normally go into Stdlib. *)
module Universe : sig
  type t =
    | No_extensions
    | Upstream_compatible
...
(** Check if a language extension is "erasable", i.e. whether it can be harmlessly
    translated to attributes and compiled with the upstream compiler. *)
...
(** Get the command line string enabling the given extension, if it's enabled; otherwise
    None *)
val get_command_line_string_if_enabled : 'a t -> string option

(** Enable and disable according to command-line strings; these raise an exception if the
...
(** Tooling support: Temporarily enable and disable language extensions; these operations
    are idempotent. Calls to [set], [enable], [disable] inside the body of the function
    argument will also be rolled back when the function finishes, but this behavior may
    change; nest multiple [with_*] functions instead. *)
val with_set : unit t -> enabled:bool -> (unit -> unit) -> unit

val with_enabled : 'a t -> 'a -> (unit -> unit) -> unit
val with_disabled : 'a t -> (unit -> unit) -> unit

(** Check if the allowable extensions are restricted to only those that are "erasable".
    This is true when the universe is set to [No_extensions] or [Upstream_compatible]. *)
...
(** Parse a command-line string and call [set_universe_and_enable_all]. Raises if the
    argument is invalid. *)
...
module For_pprintast : sig
  (** A function for wrapping a printer from "pprintast.ml" so that it will
      unconditionally print Jane Syntax instead of raising an exception when trying to
      print syntax from disabled extensions. *)
...
(** Expose the exception type raised by [assert_extension_enabled] to help the exception
    printer. *)
module Error : sig
  type error = private
    | Disabled_extension :
