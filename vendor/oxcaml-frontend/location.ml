...
let compare_position : position -> position -> int =
  ...
  | 0 ->
    (match Int.compare pos_lnum_1 pos_lnum_2 with
     | 0 ->
       (match Int.compare pos_bol_1 pos_bol_2 with
...
let compare
  ...
  | 0 ->
    (match compare_position loc_end_1 loc_end_2 with
...
let ghostify l = if l.loc_ghost then l else { l with loc_ghost = true }
...
(* The number of lines already printed after input.
   ...
   This is used by [highlight_terminfo] to identify the current position of the input in
   the terminal. This would not be possible without this information, since printing
   several warnings/errors adds text between the user input and the bottom of the
   terminal.
...
(* We use [num_loc_lines] to determine if the report about to be printed is the first or a
   follow-up report of the current "batch" -- contiguous reports without user input in
   between, for example for the current toplevel phrase. We use this to print a blank line
   between messages of the same batch.
*)
...
(* Code printing errors and warnings must be wrapped using this function, in order to
   update [num_loc_lines].

   [print_updating_num_loc_lines ppf f arg] is equivalent to calling [f ppf arg], and
   additionally updates [num_loc_lines]. *)
...
let rewrite_absolute_path path = path
;;
(* match Misc.get_build_path_prefix_map () with | None -> path | Some map ->
   Build_path_prefix_map.rewrite map path
*)

let rewrite_find_first_existing path =
  if Sys.file_exists path then Some path else None
;;
(* match Misc.get_build_path_prefix_map () with | None -> if Sys.file_exists path then
   Some path else None | Some prefix_map -> match Build_path_prefix_map.rewrite_all
   prefix_map path with | [] -> if Sys.file_exists path then Some path else None | matches
...
let rewrite_find_all_existing_dirs path =
  ...
  if ok path then [ path ] else []
;;

(* match Misc.get_build_path_prefix_map () with | None -> if ok path then [path] else [] |
   Some prefix_map -> match Build_path_prefix_map.rewrite_all prefix_map path with | [] ->
   if ok path then [path] else [] | matches -> match (List.filter ok matches) with | [] ->
   raise Not_found | results -> results
*)
...
let linenum ppf line =
  if !Clflags.locs then Format.fprintf ppf "%i" line else Format.fprintf ppf "_"
;;

let colnum ppf char =
  if !Clflags.locs then Format.fprintf ppf "%i" char else Format.fprintf ppf "_"
;;

(* Best-effort printing of the text describing a location, of the form 'File "foo.ml",
   line 3, characters 10-12'.

   Some of the information (filename, line number or characters numbers) in the location
   might be invalid; in which case we do not print it.
*)
let print_loc ~capitalize_first ppf loc =
  setup_tags ();
  let file_valid = function
    | "_none_" ->
      (* This is a dummy placeholder, but we print it anyway to please editors that parse
         locations in error messages (e.g. Emacs). *)
      true
    | "" | "//toplevel//" -> false
    | _ -> true
  in
  ...
  let file =
    (* According to the comment in location.mli, if [pos_fname] is "", we must use
       [!input_name]. *)
  ...
  if file_valid file
  then Format.fprintf ppf "%s \"%a\"" (capitalize "file") print_filename file;
  (* Print "line 1" in the case of a dummy line number. This is to please the existing
     setup of editors that parse locations in error messages (e.g. Emacs). *)
  comma ();
  ...
  then Format.fprintf ppf "%s %a" (capitalize "line") linenum startline
  else
    Format.fprintf
      ppf
      "%s %a-%a"
...
(******************************************************************************)
(* An interval set structure; additionally, it stores user-provided information at
   interval boundaries.

   The implementation provided here is naive and assumes the number of intervals to be
   small, but the interface would allow for a more efficient implementation if needed.

   Note: the structure only stores maximal intervals (that therefore do not overlap).
*)
  ...
  let of_intervals intervals =
    let pos =
      ...
      |> List.sort (fun ((_, x), k) ((_, y), k') ->
        (* Make `S come before `E so that consecutive intervals get merged together in the
           fold below *)
        let kn = function
          | `S -> 0
          | `E -> 1
...
let highlight_terminfo lb ppf locs =
  try highlight_terminfo lb ppf locs with
  | Exit -> ()
;;

(* Highlight the location by printing it again.

   There are two different styles for highlighting errors in "dumb" mode, depending if the
   error fits on a single line or spans across several lines.
   ...
   An ellipsis hides the middle lines of the multi-line error if it has more than
   [max_lines] lines.
...
(* Takes a list of lines with possibly missing line numbers.
   ...
   If the line numbers that are present are consistent with the number of lines between
   them, then infer the intermediate line numbers.

   This is not always the case, typically if lexer line directives are involved... *)
...
(* [get_lines] must return the lines to highlight, given starting and ending positions.

   See [lines_around_from_current_input] below for an instantiation of [get_lines] that
   reads from the current input.
*)
let highlight_quote
  ppf
  ...
  | Some ((leftmost, _), (rightmost, _)) ->
    let lines =
      ...
      |> List.map (fun ({ text; start_pos } as line) ->
        ...
        let line_nb =
          match
            ISet.find_bound_in iset ~range:(start_pos, end_pos), !Clflags.locs
          with
          | None, _ | _, false -> None
    ...
    (match lines with
     ...
     | [ (line, line_nb, line_start_cnum) ] ->
       ...
       (* Iterate up to [rightmost], which can be larger than the length of the line
          because we may point to a location after the end of the last token on the line,
          for instance:
          {[
            token
                      ^
       ...
       for i = 0 to rightmost.pos_cnum - line_start_cnum - 1 do
         ...
         then
           (* For alignment purposes, align using a tab for each tab in the source code *)
           if line.[i] = '\t'
           then Format.pp_print_char ppf '\t'
           else Format.pp_print_char ppf ' ';
         if ISet.is_end iset ~pos <> None then Format.fprintf ppf "@}"
       done;
       Format.fprintf ppf "@}@,"
     | _ ->
       (* Multi-line error *)
...
let lines_around_from_lexbuf
  ...
  then
    (* Do nothing if the buffer does not contain the input (because it has been refilled
       while lexing it) *)
    []
  else (
    let pos = ref 0 in
...
(* A [get_lines] function for [highlight_quote] that reads from the current input. *)
let lines_around_from_current_input ~start_pos ~end_pos =
  ...
  | None, _, filename ->
    (* A situation where we have no input buffer and no phrase buffer is when the compiler
       is getting the binary AST directly as input. *)
    (* Be a bit defensive, and do not try to open one of the possible [!input_name] values
       that we know do not denote valid filenames. *)
    let file_valid =
      match filename with
...
type report_printer =
  { (* The entry point *) pp :
      report_printer -> Format.formatter -> report -> unit
  ; pp_report_kind :
...
let is_dummy_loc loc =
  (* Fixme: this should be just [loc.loc_ghost] and the function should be inlined below.
     However, currently, the compiler emits in some places ghost locations with valid
     ranges that should still be printed. These locations should be made non-ghost -- in
     the meantime we just check if the ranges are valid. *)
  loc.loc_start.pos_cnum = -1 || loc.loc_end.pos_cnum = -1
;;

(* It only makes sense to highlight (i.e. quote or underline the corresponding source
   code) locations that originate from the current input.
   ...
   - more generally, if some code uses the compiler-libs API and feeds it locations that
     do not match the current values of [!Location.input_name], [!Location.input_lexbuf];

   - when calling the compiler on a .ml file that contains lexer line directives
     indicating an other file. This should happen relatively rarely in practice -- in
     particular this is not what happens when using -pp or -ppx or a ppx driver.
*)
...
let batch_mode_printer : report_printer =
  let pp_loc _self report ppf loc =
    let tag =
      ...
      | Report_warning_as_error _ | Report_alert_as_error _ | Report_error ->
        "error"
  ...
  let pp self ppf report =
    ...
    (* Make sure we keep [num_loc_lines] updated. The tabulation box is here to give
       submessage the option to be aligned with the main message box
    *)
    print_updating_num_loc_lines
      ...
      (fun ppf () ->
        Format.fprintf
          ...
          Format.pp_open_tbox
          ()
          (self.pp_main_loc self report)
          report.main.loc
          (self.pp_report_kind self report)
          report.kind
          Format.pp_set_tab
          ()
          (self.pp_main_txt self report)
          report.main.txt
          (self.pp_submsgs self report)
          report.sub
  ...
  let pp_submsgs self report ppf msgs =
    List.iter
      (fun msg -> Format.fprintf ppf "@,%a" (self.pp_submsg self report) msg)
      msgs
  in
  let pp_submsg self report ppf { loc; txt } =
    Format.fprintf
      ppf
      "@[%a  %a@]"
      (self.pp_submsg_loc self report)
      loc
      (self.pp_submsg_txt self report)
      txt
  in
  let pp_submsg_loc self report ppf loc =
    if not (is_dummy_loc loc) then pp_loc self report ppf loc
  in
...
let terminfo_toplevel_printer (lb : lexbuf) : report_printer =
  let pp self ppf err =
    setup_tags ();
    (* Highlight all toplevel locations of the report, instead of displaying the main
       location. Do it now instead of in [pp_main_loc], to avoid messing with Format
       boxes. *)
  ...
  let pp_submsg_loc _ _ ppf loc =
    if not (is_dummy_loc loc) then Format.fprintf ppf "%a:@ " print_loc loc
  in
...
let best_toplevel_printer () =
  ...
  | Terminfo.Good_term, Some lb -> terminfo_toplevel_printer lb
  | _, _ -> batch_mode_printer
;;

(* Creates a printer for the current input *)
let default_report_printer () : report_printer =
  if !input_name = "//toplevel//"
  then best_toplevel_printer ()
  else batch_mode_printer
;;
...
(******************************************************************************)
(* Reporting warnings: generating a report from a warning number using the information in
   [Warnings] + convenience functions. *)
...
let auto_include_alert lib =
  let message =
    Format.asprintf
      "OCaml's lib directory layout changed in 5.0. The %a subdirectory has \
       been automatically added to the search path, but you should add %a to \
       the command-line to silence this alert (e.g. by adding %a to the list \
       of libraries in your dune file, or adding %a to your %a file for \
  ...
  let message =
    Format.asprintf
      "Running %a where the first argument is an implicit basename with no \
       extension (e.g. %a) is deprecated. Either rename the script (%a) or \
       qualify the basename (%a)"
      Style.inline_code
      program
      Style.inline_code
      (program ^ " script-file")
      Style.inline_code
      (program ^ " script-file.ml")
      Style.inline_code
      (program ^ " ./script-file")
  in
  let alert =
