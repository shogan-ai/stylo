  ...
  let rec walk_both items tokens =
    ...
    | t :: tokens ->
      (match t.Tokens.desc with
       ...
       | Token _ ->
         (* No tokens appart from SEMISEMI at this level, and we removed them already. *)
         assert false
       | Child_node ->
         (match items with
          | [] -> assert false
...
let normalize_use_file_semisemi (items, tokens) =
  let rec walk_both items tokens rev_items rev_tokens =
    match tokens with
    | [] -> (* CR mdelvecchio: Any checks here? *)
      List.rev rev_items, List.rev rev_tokens
    | t :: tokens ->
      (match t.Tokens.desc with
       | Token (EOF, _) | Comment _ ->
         walk_both items tokens rev_items (t :: rev_tokens)
       | Token _ -> assert false
       | Child_node ->
         (match items with
          | [] -> assert false
          | [ last ] ->
            walk_both [] tokens (last :: rev_items) (t :: rev_tokens)
          | Ptop_def (this_str, this_tokens)
            :: Ptop_def (next_str, next_tokens)
            :: items ->
            let between, tokens =
              let rec loop between_rev tokens =
                match tokens with
                | [] -> assert false
                | { desc = Child_node; _ } :: tokens ->
                  List.rev between_rev, tokens
                | token :: tokens -> loop (token :: between_rev) tokens
              in
              loop [] tokens
            in
            let merged =
              Ptop_def
                (this_str @ next_str, this_tokens @ between @ next_tokens)
            in
            walk_both (merged :: items) (t :: tokens) rev_items rev_tokens
          | this :: (next :: _ as items) ->
            let this_loc =
              match this with
              | Ptop_def ({ pstr_loc; _ } :: _, _) -> pstr_loc
              | Ptop_def ([], _) -> assert false
              | Ptop_dir { pdir_loc; _ } -> pdir_loc
              | Ptop_lex { plex_loc; _ } -> plex_loc
            in
            let rec ends_with_pstr_attribute = function
              | { pstr_desc = Pstr_attribute _; _ } :: _ -> true
              | [] -> false
              | _ :: tl -> ends_with_pstr_attribute tl
            in
            let require_semisemi =
              match this, next with
              | _, Ptop_def ({ pstr_desc = Pstr_eval _; _ } :: _, _) -> true
              | Ptop_def (str, _), _ when ends_with_pstr_attribute str -> false
              | Ptop_def _, (Ptop_dir _ | Ptop_lex _) -> true
              | _ -> false
            in
            let rev_items, rev_tokens =
              if not require_semisemi
              then this :: rev_items, t :: rev_tokens
              else (
                let semisemi = semisemi ~optional:false this_loc.loc_end in
                match this with
                | Ptop_def _ | Ptop_dir _ ->
                  this :: rev_items, semisemi :: t :: rev_tokens
                | Ptop_lex dir ->
                  ( Ptop_lex
                      { dir with plex_tokens = dir.plex_tokens @ [ semisemi ] }
                    :: rev_items
                  , t :: rev_tokens ))
            in
            walk_both items tokens rev_items rev_tokens))
  in
  let tokens_no_semi = Utils.without ~token:SEMISEMI tokens in
  walk_both items tokens_no_semi [] []
;;
...
let exp_no_trailing e =
  ...
  | Pexp_record (_, fields) | Pexp_record_unboxed_product (_, fields)
    ...
    (* at first glance seems like the wrong level to do this, but the semis are actually
       part of the expression tokens, not the [record_field]. *)
