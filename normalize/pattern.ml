...
let remove_parens p =
  ...
  | Ppat_parens { pat = p'; optional = _ } ->
    ...
    let comments_before_p' =
      Base.List.take_while ~f:(fun t -> not (Tokens.is_child t)) p.ppat_tokens
      |> List.filter Tokens.is_comment
    in
    let comments_after_p' =
      Base.List.drop_while ~f:(fun t -> not (Tokens.is_child t)) p.ppat_tokens
      |> List.tl
      |> List.filter Tokens.is_comment
    in
    { p' with
      ppat_tokens = comments_before_p' @ p'.ppat_tokens @ comments_after_p'
    }
  | _ -> assert false
;;
    ...
    (match parent, p'.ppat_desc with
     ...
     | _ ->
       (* parens are not mandatory but we might still optionally keep them. Removing them
          ...
          comments from one outside the parentheses to inside. *)
       (match p'.ppat_desc with
...
let map mapper (parent : Context.parent) pat =
  let pat =
    ...
    | Expr _, Ppat_parens { pat = { ppat_desc = Ppat_or _; _ }; _ }
      when pat.ppat_attributes = [] -> remove_parens pat
