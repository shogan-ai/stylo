...
(* Assumption: there is no overlap of the form
   {v [   (  ]  ) v}
   . *)
...
let pos_to_lc (pos : Lexing.position) = pos.pos_lnum, pos.pos_cnum - pos.pos_bol
...
let relate_selection ~loc (sel_start_l, sel_start_c) (sel_end_l, sel_end_c) =
  ...
  if node_start_l > sel_end_l
     || node_end_l < sel_start_l
     || (node_start_l = sel_end_l && node_start_c > sel_end_c)
     || (node_end_l = sel_start_l && node_end_c < sel_start_c)
  then Disjoint
  else if node_start_l > sel_start_l
          || node_end_l < sel_end_l
...
let run start stop source =
  ...
  | Ok cst ->
    ...
    in
    (match get_tokens#from_structure range cst with
     | Some tokens ->
       PP.seq tokens
...
let cmd =
  ...
  in
  let+ start = info [ "start" ] |> opt (some' pos_conv) None |> required
  and+ stop = info [ "stop" ] |> opt (some' pos_conv) None |> value in
  In_channel.input_all Stdlib.stdin |> run start stop
;;
