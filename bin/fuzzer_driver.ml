...
let batch_by ~size seq =
  ...
  let rec take acc n seq =
    if n = 0
    then Seq.Cons (List.rev acc, start seq)
    else
      match seq () with
  ...
  and start seq () =
    match seq () with
    | Seq.Nil -> Seq.Nil
    | Seq.Cons (x, xs) -> take [ x ] (size - 1) xs
  in
  start seq
;;
...
let overlapping_force jobs seq =
  ...
  let rec reconstruct queue seq () =
    ...
    | Some (x, queue') ->
      ...
      | Seq.Cons (x', seq') -> Seq.Cons (x, reconstruct (push queue' x') seq')
  in
...
let consume_batch = function
  ...
  | Some (fn, pid) ->
    ...
    | _ -> ()
;; (* exited abnormally, keeping the file and its sidecars *)

(* Unlike [Filename.get_temp_dir_name () ^ "/formatpinata"], this directory is
   unique to this run: previous runs, possibly concurrent, cannot interfere with
   it. *)
    ...
    | inputs ->
      ...
      Out_channel.with_open_text path (fun oc ->
        List.iter
          ...
          inputs);
      (* Flags are passed through verbatim: no option here has any effect on the
         flags sent to [cmd], other than its own. *)
      let flags =
        (if syntax_quotations then [ "--syntax-quotations" ] else [])
        @ (if ignore_output_syntax_errors
           then [ "--ignore-output-syntax-errors" ]
           else [])
      ...
      let pid =
        Unix.create_process cmd argv Unix.stdin Unix.stdout Unix.stderr
      in
      Some (path, pid)
  in
  seq
  |> (* Group by batches of appropriate size *) batch_by ~size:batch_size
  |> (* Launch a process for each batch *) Seq.map start_batch
  |> (* Force sequence enough items ahead to kick [jobs] processes ahead *)
     overlapping_force
       jobs
  |> (* Collect the results *) Seq.iter consume_batch;
  (* Clean up, unless some batch files were kept because of a failure. *)
  (try Unix.rmdir temp_dir with
   | _ -> ())
;;
...
let run
  ...
  input_file
  =
  In_channel.with_open_text input_file
  @@ fun ic ->
  let seq =
    Seq.of_dispenser (fun () ->
      Option.map Bytes.unsafe_to_string (Std.read_input ic))
  in
  check
    ~cmd
    ~batch_size
...
let driver_cmd =
  ...
  let+ cmd =
    info
      [ "cmd" ]
      ~doc:
        "Command to run on each batch of sentences. It must support stylo's \
         $(b,fuzz) subcommand, as it is invoked as $(b,CMD fuzz [flags] \
         BATCH_FILE)."
      ~docv:"CMD"
  ...
  and+ batch_size =
    info
      [ "size"; "batch-size" ]
      ~doc:"Number of sentences per batch."
      ~docv:"N"
  ...
  and+ jobs =
    info
      [ "jobs"; "j" ]
      ~doc:"Number of stylo processes to run in parallel."
      ~docv:"N"
    |> opt int 1
    |> value
  and+ syntax_quotations =
    info [ "syntax-quotations" ] ~doc:"Pass $(b,--syntax-quotations) to stylo."
    |> flag
    |> value
  ...
  and+ idempotence_check =
    info
      [ "idempotence-check" ]
      ~doc:
        "Pass $(b,--idempotence-check) to stylo: check that formatting the \
  ...
  and+ failures_dir =
    info
      [ "failures-dir" ]
      ~doc:
        "Pass $(b,--failures-dir DIR) to stylo: failing entries are saved (one \
         file per entry) in $(i,DIR) instead of a $(i,<batch>.failures) \
         directory."
      ~docv:"DIR"
    |> opt (some filepath) None
    |> value
  and+ input_file =
    info [] ~doc:"File holding the corpus of NUL-separated sentences to check."
  ...
  let info =
    Cmd.info
      ...
      ~man:
        [ `S "DESCRIPTION"
        ; `P
            ("fuzzer_driver reads a corpus of NUL-separated sentences from \
              $(b,FILE), splits it into batches and invokes stylo's $(b,fuzz) \
              subcommand on each batch, in parallel. Batches for which stylo \
              reports a failure are kept on disk for inspection, along with \
              the offending entries, saved one per file in a \
              $(i,<batch>.failures) directory (or in the directory given by \
              $(b,--failures-dir), if any); the others are deleted.")
        ]
  in
