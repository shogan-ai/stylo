...
(* let temp_path_index name = let name = Filename.remove_extension (Filename.basename
   name) in Scanf.sscanf_opt name "formatpinata_%d-%d" (fun _id index -> index)
*)
...
let check ~ocamlformat_command ~jobs ~batch_size seq =
  ...
  |> (* Launch a process for each batch *) Seq.map
       (start_batch ~ocamlformat_command)
  |>
