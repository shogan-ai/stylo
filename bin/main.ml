...
module Arg = struct
  ...
  let single_file = pos 0 (some filepath) None (info []) |> required
  let files = pos_all filepath [] (info []) |> value
  ...
  let idempotence_check =
    info
      [ idempotence_check_arg_name ]
      ~doc:
        "Check that calling the tool again on the output produces the same \
         result."
    |> flag
    |> value
  ;;
  ...
  let ignore_syntax_errors =
    info [ "ignore-output-syntax-errors" ] |> flag |> value
  ;;

  let width = info [ "width"; "w" ] |> opt int 80 |> value

  let debug =
    let doc =
      Format.sprintf
        "Dumps stylo's internal structures at various stages of the \
         pipeline.For a given input file FILE the files which can appear are: \
  ...
  let failures_dir =
    info
      [ failures_dir_arg_name ]
      ~doc:
        "Save failing entries (one file per entry) in the given directory, \
         instead of a $(i,<batch>.failures) directory next to the batch file. \
         Sharing a directory between batches is fine: files are prefixed with \
         the name of the batch they come from."
      ~docv:"DIR"
    |> opt (some filepath) None
    |> value
  ;;
...
let fuzzer_batch ~quiet ~idempotence_check ~failures_dir fn =
  ...
  (* Failing entries are saved, one file per entry; entries are stripped of
     their entrypoint prefix and get a [.ml] or [.mli] extension so that they
     can be replayed with [stylo style --idempotence-check FILE] directly. By
     default they land in a [fn ^ ".failures"] directory next to the batch file,
     but they can be gathered in a directory shared by all the batches, in which
     case files are prefixed with the name of the batch they come from. *)
  let failures_dir, failure_prefix =
    match failures_dir with
    | Some dir -> dir, Filename.remove_extension (Filename.basename fn) ^ "-"
    | None -> fn ^ ".failures", ""
  in
  ...
  let rec loop_entries lnum ic =
    ...
    | Some bytes ->
      ...
      | Error `Output_parse_error _ ->
        (* when [--ignore-output-syntax-errors] is passed we explicitely skip
           those as we're only interested in idempotence issues when the flag is
           passed. *)
        if not quiet then save_failure entrypoint_and_src;
        loop_entries next_lnum ic
      | Error e ->
  ...
  then
    Format.eprintf
      "Checked %d entries from %s: %d did not parse, %d failing entries saved \
       in %s@."
      !entries_checked
      fn
      !parse_errors
    ...
    Format.eprintf
      "Checked %d entries from %s: %d did not parse@."
      !entries_checked
      fn
      !parse_errors;
  if !has_errors then Cmd.Exit.some_error else Cmd.Exit.ok
;;
  ...
  List.iter
    (fun (fkind, fn) ->
      ...
      | Error `Not_idempotent ->
        Format.eprintf
          "%s: `--idempotence-check` failure: output is not stable@."
          fn;
        has_error := true
      ...
      | Ok output ->
        let pp oc =
          ...
          flush oc
        in
        if not inplace then pp stdout else Out_channel.with_open_text fn pp)
    fns;
  if !has_error then Cmd.Exit.some_error else Cmd.Exit.ok
;;
