...
(* N.B. [stringf] is important here: suffixed number come out of the lexer as a single
   token. We can't use ^^ here. *)
  ...
  module Arrow = struct
    ...
    let pp_arg
      ...
      let arg =
        ...
        | Labelled s ->
          let lbl, _ =
            Preceeding.group_with preceeding (string s ^^ S.colon)
          in
          lbl
          ^^ break 0
          ^^ nest 2
...
and Pattern : sig
  ...
  and pp_desc ~preceeding pat_flatness p =
    ...
    | Ppat_alias (p, alias) ->
      ...
      pp ?preceeding p
      ^^
         (* try to fit the alias at the end of the current line, but prioritize keeping
            the [as alias] together over filling the line. *)
         group
           (break 1
            ^^ pre_nest
    ...
    | Ppat_construct (lid, arg) ->
      pp_construct ~preceeding p.ppat_tokens lid arg
  ...
  and pp_construct ~preceeding tokens name arg_opt =
    ...
    | Some ([ (newtype, Some jkind) ], arg_pat)
      ...
      (* We could decide to "normalize" this case. Here we are careful because we don't
         want to trigger if the user wrote
         {[
...
and Expression : sig
  ...
  let rec needs_space_if_prefixed e =
    ...
    | Pexp_send (lhs, _)
    | Pexp_field (lhs, _)
    | Pexp_index_op { seq = lhs; _ } -> needs_space_if_prefixed lhs
    | _ -> false
  ;;
  ...
  and pp_desc ~preceeding exp =
    ...
    | Pexp_sequence (e1, e2) ->
      (* FIXME: ext_attr not at the beginning, the token synchronisation is going to have
         issues. *)
      pp ?preceeding e1 ^^ !!S.semi ^^ hardline ^^ pp e2
    | Pexp_seq_empty e ->
      pp ?preceeding e
      ^^ S.semi
      (* the following is needed if we're at the end of a record (or override) field for
         instance.
    ...
    | Pexp_letexception (ec, body) ->
      ...
      in
      let binding =
        let_exn ^/^ pre_nest (nest 2 (Extension_constructor.pp ec) ^/^ S.in_)
      in
  ...
  and pp_index_op ~preceeding nb_semis kind seq op indices assign =
    let open_, close, spaces_around_indices =
      let has_quotation =
        (* Over-approximation: quotations are only problematic in the first and last
           position (because without spaces they change the way the opening/closing is
           lexed). But ... whatever. *)
    ...
    let pre_nest = Preceeding.implied_nest preceeding in
    let access =
      group (pp ?preceeding seq ^^ break 0 ^^ pre_nest (group dotop ^^ open_))
  ...
  and pp_parens ~preceeding ~optional exp =
    ...
    | _ ->
      ...
      in
      let indent =
        (* Special case which reproduces inconsistencies in oxcamlformat's output. *)
        match exp.pexp_desc with
        | Pexp_function ([], _, _) -> 2
        | Pexp_function _ -> 0
        | _ -> 1
  ...
  and pp_field ?preceeding ?(unboxed = false) e lid =
    ...
    let pre_nest = Preceeding.implied_nest preceeding in
    group (pp ?preceeding e ^^ dot ^^ pre_nest (longident lid.txt))
...
and Application : sig
  ...
  let pp ?(extra_nest = Fun.id) ?(indent = 2) f args =
    ...
    (* N.B. the 2 (instead of [indent]) below is to reproduce ocamlformat's behavior. But
       it feels *wrong* and should eventually be fixed? *)
    ^^ extra_nest
    @@ nest 2 trailing
  ;;
end
...
and Case : sig
  ...
  (* we only nest the pattern, not the pipes (there might be several due to or-patterns). *)
  let pp_pattern pipe p =
    let rec split_top_or p =
      match p.ppat_desc with
      | Ppat_or (p1, p2) ->
        (* N.B. we're sure there's no attribute here, one needs parens to attach an
           attribute to an or pattern in ocaml. So the attribute would be on a Ppat_parens
           above the Ppat_or. *)
...
and Argument : sig
  ...
  let pp_preceeded
    ...
    | Parg_unlabelled { legacy_modes; arg; typ_constraint; modes } ->
      let lpre, pre_nest = Preceeding.extend preceeding S.lparen ~indent:1 in
      pp_generic
        ~preceeding:lpre
        legacy_modes
...
and Function_body : sig
  (** [pp_parts (Pfunction_cases _)] is ["function", cases] [pp_parts (Pfunction_body e)]
      is [<empty>, e] *)
  ...
  let pp_cases ?preceeding cases ext_attrs =
    ...
    in
    pre_function_, pre_nest (softest_line ^^ Case.pp_cases cases)
  ;;

  let pp_parts ?preceeding fb =
    match fb.pfb_desc with
    | Pfunction_body e -> empty, Expression.pp ?preceeding e
    | Pfunction_cases (cases, ext_attrs) -> pp_cases ?preceeding cases ext_attrs
  ;;
...
and Record : sig
  ...
  let pp_field
    ...
    @@ nest
         3
         (* oxcamlformat indents by 3 instead of 2 here, why? *)
         (group
...
and Constructor_argument : sig
  ...
  let pp_args = function
    | Pcstr_tuple args -> separate_map (break 1 ^^ S.star ^^ break 1) pp args
  ...
  let pp_kind = function
    | Pext_decl (vars, args, res) ->
      ...
      (match args, res with
       ...
       | args, None -> break 1 ^^ S.of_ ^/^ Constructor_argument.pp_args args
       | Pcstr_tuple [], Some ct ->
...
and Class_type_declaration : sig
  ...
  let pp_list =
    Class_infos.pp_list Class_type.pp ~keywords:[ S.class_; S.type_ ]
  ;;
end
...
and Class_expr : sig
  ...
  and pp_value lbl mut = function
    ...
    | Cfk_concrete (over, vb) ->
      let start = [ S.val_ ^^ override_ over; mutable_ mut ] in
  ...
  and pp_method lbl priv = function
    | Cfk_virtual ct ->
      ...
      ^/^ Core_type.pp ct
    | Cfk_concrete (over, vb) ->
      let start = [ S.method_ ^^ override_ over; private_ priv ] in
      Value_binding.pp ~start vb ~add_in:false
  ...
  let as_rhs
    ...
    | Pmty_typeof (attrs, me) ->
      ...
      (match Module_expr.as_rhs me with
       ...
       | Three_parts rhs ->
         Three_parts { rhs with start = prefix ^/^ rhs.start })
...
and Signature : sig
  ...
  let pp_item_desc = function
    ...
    | Psig_kind_abbrev (name, k) ->
      S.kind_abbrev__ ^/^ string name.txt ^/^ S.equals ^/^ Jkind_annotation.pp k
      |> group
  ;;
  ...
  let pp
    ...
    let params, equal_sign, rhs =
      ...
      | With_params (params, mty, modes) ->
        ...
        in
        List.map Functor_parameter.pp params, S.colon, rhs
...
and Layout_module_binding : sig
  type rhs =
    ...
    | Three_parts of
      ...
      (** Meant for [struct/sig .. end]: we try to keep [start] on the same line as what
        preceeds it, [main] is indented, [stop] is not.

        N.B. also used for value bindings in cases such as [let f = function ...], there
        [start = function] and [stop] is empty. *)
  ...
  (* Beware: [equal_sign] is never part of [rhs], we have to insert it explicitely,
     whereas [constraint] sometimes include the "delimiter": a colon or an at. We could
     have had a [colon_sign] for symmetry, but pffff...

     Also: "sometimes" == it does include it when it is [Single_part], but doesn't for
     [Three_parts] as there it is always a colon, so we do it here.
  ...
  let pp
        ...
        group
          (main
           ^/^ separate_map
                 ...
                 params)
    in
    (* FIXME: the ':' and '=' (and friends) should be attached to what preceeds them if we
       want to match oxcamlformat's janestreet style. This is currently not doable as the
       colon is part of the constraint when the constraint is Single_part. *)
    ...
    | None, Some (Three_parts { start; main; stop }) ->
      let bindings_and_main =
        group (bindings ^/^ pre_nest @@ group (nest 2 equal_sign ^/^ start))
        ^?^ pre_nest
        @@ nest 2 main
      in
      group (group bindings_and_main ^?^ pre_nest stop)
    | Some (Three_parts { start; main; stop }), None ->
      let bindings_and_main =
        group (bindings ^/^ pre_nest @@ group (nest 2 S.colon ^/^ start))
        ^/^ pre_nest
        @@ nest 2 main
      in
      group (group bindings_and_main ^/^ pre_nest stop)
    | Some (Three_parts { start; main; stop }), Some (Single_part doc) ->
      ...
      group (group bindings_and_main ^/^ pre_nest (group stop_and_rhs))
    | Some (Single_part doc), Some (Three_parts { start; main; stop }) ->
      let bindings_cstr_main =
        group
          (bindings
           ^/^ pre_nest
           @@ group (nest 2 (group (doc ^/^ equal_sign)) ^/^ start))
        ^/^ pre_nest
        @@ nest 2 main
      in
      group (group bindings_cstr_main ^/^ pre_nest stop)
    | Some (Three_parts typ), Some (Three_parts exp) ->
      let bindings_and_typ =
        group (bindings ^/^ pre_nest @@ nest 2 (group (S.colon ^/^ typ.start)))
        ^/^ pre_nest
        @@ nest 2 typ.main
      in
      let eq_and_exp =
        ...
        ^?^ exp.stop
      in
      group (group bindings_and_typ ^/^ pre_nest eq_and_exp)
  ;;
  ...
  let pp
    ...
    bound
    =
    (* Here we assume that [preceeding] cannot be [Some _] at the same time as [pre_text]
       or [pre_doc]. *)
    pp
      ?preceeding
      ?params_indent
...
and Value_binding : sig
  ...
  let layout ?preceeding ~keyword ~params ?constraint_ ?rhs bound in_kw =
    ...
    | None, Some (Three_parts { start; main; stop }) ->
      let bindings_and_main =
        group (bindings ^/^ pre_nest @@ nest 2 (group (S.equals ^/^ start)))
        ^/^ pre_nest
        @@ nest 2 main
      in
  ...
  let rhs e =
    ...
    | Pexp_function _ ->
      ...
      (* N.B. there can be attributes here in unexpected cases, for instance:
         [fun _ -> x; [@attr]] the semicolon make the attribute attach to the function,
         not the body. *)
...
and Module_binding : sig
  ...
  let modal_constraint constr mode_l : Layout_module_binding.rhs option =
    ...
    | Some (Single_part mty), modes ->
      Some (Single_part (S.colon ^/^ mty |> with_modes ~modes))
...
(* (**
   ...
   *)
   ...
   and directive_argument_desc = | Pdir_string of string | Pdir_int of string * char
   option | Pdir_ident of Longident.t | Pdir_bool of bool
*)
