...
type whitespace =
  ...
  | Break of int * softness
    (** [Break (n, softness)] prints as [n] spaces in flat mode, and behaves as a
      [Line_break softness] otherwise. *)
  | Non_breakable (** a simple space. *)

(** Some leaves of the document can "vanish" under certain conditions. For instances
    parentheses around a tuple might be present if the tuple is spread over several lines,
    but absent if it fits on one line. *)
type 'a can_vanish =
...
(** A single token. Must not contain linebreaks. *)
val string : string -> t

(** For tokens (e.g. string literals) which might contain linebreaks. *)
val fancy_string : string -> t

(** For complex subtrees which map to a single input token. For instance string literals
    that have been specially formatted (e.g. wrapped, etc). *)
val formatted_string : t -> t

(** {2 Whitespace documents}

    Refer to {!softness} and {!whitespace} for more information about the various kind of
    whitespaces. *)
...
(** An explicit hint for the comments insertion algorithm to flush comments at this point:
    it allows for floating comments to be displayed as such, whereas they would otherwise
    be attached to what follows them.

    Renders as [empty] if there are no comments to insert.

    The [Condition.t] will evaluate to true if some comments were inserted, false
...
(** Instruct the engine to change the indentation level for the given document.
    ...
    The [vanish] parameter is used to make the nest node (but not its content) vanish. An
    situation where this is useful is if the node is between optional parentheses. One
    wants to increase the indentation level when they are present, but not when they
    vanish. This can be achieved by passing the same [Condition.t] to all three nodes. *)
val nest : ?vanish:Condition.t -> int -> t -> t

(** If the given doc fits on the current, then it is printed on one line (i.e. the
    printing engine enters its "flat" mode) and all the groups inside are ignored (as we
    will already be in flat mode), otherwise printing continues in normal mode.

    [margin] is used to extend document's requirement when deciding whether to enter flat
    mode or not, but it doesn't change the requirement of the group itself (and so doesn't
    propagate nor affect the parent's flatness).

    The value of [flatness] will be decided by the printer when traversing the group and
    can then be part of a [Condition.t]. N.B. if the condition is checked before the group
    is traversed, it will be assumed to be non-flat. *)
