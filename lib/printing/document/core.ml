...
module Requirement
  (* : sig type t
     ...
     val ( + ) : t -> t -> t (** Beware: not commutative! Keeps the indent of
     the first operand. *)

     val nest : int -> t -> t

     val to_int_including_indent : current_indent:int -> t -> int end *)
  = struct
  type expr =
    | Infinity
    | Const of int
  ...
  let rec eval = function
    ...
    | Add (a, b) ->
      let a = eval a in
      let b = eval b in
      if a = max_int || b = max_int then max_int else a + b
    | If (c, then_, else_) -> eval (if Condition.eval c then then_ else else_)
  ;;
...
let requirement = function
  | Empty -> Req.of_int 0
  | Directive pt | Comment { doc = pt; _ } -> pseudo_token_req pt
  | Token { vanishing_cond; value = pt } ->
    let pt_req = pseudo_token_req pt in
    Req.if_ vanishing_cond ~then_:(Const 0) ~else_:pt_req.compact_req
  | Whitespace { vanishing_cond; value = ws } ->
    let ws_req = ws_req ws in
    Req.if_ vanishing_cond ~then_:(Const 0) ~else_:ws_req.compact_req
...
let vanishing_whitespace cond = function
  ...
  | Whitespace { vanishing_cond = other_cond; value } ->
    Whitespace { vanishing_cond = Condition.(cond || other_cond); value }
  | _ -> invalid_arg "Document.vanishing_whitespace"
;;

let flush_comments ~pull_preceeding_comments:pull ~ws_before ~ws_after =
...
let fancy_string s =
  Token { vanishing_cond = False; value = pseudo_of_string s }
;;

let formatted_string t =
  Token { vanishing_cond = False; value = Complex (requirement t, t) }
;;
