...
let incr_col st i = { st with column = st.column + i }
...
module Buffer : sig
  ...
  let add_spaces t n = t.delayed_ws <- t.delayed_ws + n
...
let newline st soft t =
  ...
  | Soft, Follows_blank_line | Softest, (Is_empty | Follows_blank_line) -> st
  | _ ->
    Buffer.newline t;
    let line =
      match st.line with
...
let whitespace buf state indent flat = function
  | Break (spaces, soft) ->
    ...
    else newline state soft buf
  | Line_break soft ->
    assert (not flat);
    newline state soft buf
  | Non_breakable -> add_spaces buf state indent 1
;;

let rec pretty buf state indent flat = function
  | Empty | Comments_flushing_hint _ -> state
  | Token { vanishing_cond = cond; _ } | Whitespace { vanishing_cond = cond; _ }
    when Condition.check cond -> state
  | Token { value = pseudo; _ } | Comment { doc = pseudo; _ } ->
    pp_pseudo buf state indent flat pseudo
  ...
  | Group (req, margin, flat_track_opt, t) ->
    ...
    in
    Option.iter
      (fun flatness -> (flatness : flatness :> bool ref) := flat)
      flat_track_opt;
    pretty buf state indent flat t
...
let to_string ~width d =
  ...
  let init = { max_width = width; column = 0; line = Is_empty } in
