...
type whitespace =
  ...
  | Break of int * softness
    (** [Break (n, softness)] prints as [n] spaces in flat mode, and behaves as
        a [Line_break softness] otherwise. *)
  | Non_breakable (** a simple space. *)

(** Some leaves of the document can "vanish" under certain conditions. For
    instances parentheses around a tuple might be present if the tuple is spread
    over several lines, but absent if it fits on one line. *)
type 'a can_vanish =
...
(** For complex subtrees which map to a single input token. For instance string
    literals that have been specially formatted (e.g. wrapped, etc). *)
...
(** An explicit hint for the comments insertion algorithm to flush comments at
    ...
    false otherwise. N.B. the condition will necessarily get it's final value
    before the printing engine runs, so it can be used to make elements
    preceeding the comments vanish, not just elements who follow them. *)
val flush_comments
...
(** Instruct the engine to change the indentation level for the given document.
    ...
    vanish. An situation where this is useful is if the node is between optional
...
(** If the given doc fits on the current, then it is printed on one line (i.e.
    ...
    [margin] is used to extend document's requirement when deciding whether to
    enter flat mode or not, but it doesn't change the requirement of the group
    itself (and so doesn't propagate nor affect the parent's flatness).

    The value of [flatness] will be decided by the printer when traversing the
    group and can then be part of a [Condition.t]. N.B. if the condition is
    checked before the group is traversed, it will be assumed to be non-flat. *)
