...
let parens d = (* fixme: nest 1?? *) group (S.lparen ^^ nest 1 d ^^ S.rparen)
...
let constant c =
  ...
  | Pconst_float (sign, nb, None) | Pconst_integer (sign, nb, None) ->
    optional string sign ^^ string nb
...
module rec Attribute : sig
  ...
  let pp_attr_name ids = separate_map S.dot string ids.txt
  ...
  let pp
    ...
    =
    pp (if item then S.lbracket_atat else S.lbracket_at) attr_name attr_payload
  ;;
...
and Extension : sig
  ...
  let pp ?preceeding ?(floating = false) (ext_name, ext_payload, _tokens) =
    ...
    | PString (s, delim) ->
      ...
      in
      Preceeding.group_with preceeding s |> fst
...
and Type_constructor : sig
  ...
  val pp_decl : Tokens.seq -> ptype_params -> t -> t
  ...
  let pp
    (type a)
    ?preceeding
    ~(pp_arg : ?preceeding:Preceeding.t -> a -> t)
    delims
    (args : a list)
    ty_doc
    =
    ...
    | arg :: args ->
      ...
      | _ ->
        let left, pre_nest = Preceeding.(preceeding + tight left ~indent:2) in
        let comma_pre = Preceeding.spaced S.comma in
        let left_and_args =
          List.fold_left
            (fun acc arg ->
  ...
  let pp_core_type ?(class_ = false) ?preceeding tokens args lid =
    let delims =
      Parens
        { omit_when_single_param = args <> [] && not (starts_with LPAREN tokens)
        }
    in
  ...
  (* '[' followed by '<' will be parsed as LBRACKETGREATER without a space between the two symbols. A similar thing happens for '>' followed by ']'
     *)
...
and Core_type : sig
  ...
  module Arrow : sig
    ...
    val components
      :  core_type
      -> (bound_ty_var list * arrow_arg list * (modes * modes * core_type))
           option
  ...
  val pp_poly_bindings : ?preceeding:Preceeding.t -> bound_ty_var list -> t
  ...
  module Arrow = struct
    ...
    let components ct =
      ...
      | Ptyp_poly
          ...
          , { ptyp_desc =
                Ptyp_arrow
                  { domain = dom; codom_legacy_modes; codom_type; codom_modes }
    ...
    let pp_arg
      ...
      let arg =
        ...
        | Labelled s ->
          ...
          ^^ break 0
          ^^ nest 2
          @@ pre_nest (pp_arg_moded_ty aa_legacy_modes aa_modes aa_type ())
        | Optional s ->
          let lbl =
            if List.exists
                 (function
    ...
    let pp
      ...
      let mk_pre_doc token =
        (* TODO: swap vanishing nbsp with break, so we can use
           [Preceeding.spaced] *)
      ...
      let first_pre, pre_nest =
        ...
        then
          let pre, pre_nest =
            Preceeding.(preceeding + tight ~indent:3 (mk_pre_doc S.colon))
          in
          Some pre, pre_nest
      ...
      let params, preceeding, first_pre_nest =
        ...
        | _ ->
          ...
          let dot_pre = Preceeding.tight ~indent:3 (mk_pre_doc S.dot) in
          polys ^^ break 0, Some dot_pre, pre_nest
      in
      let arrow_pre = Preceeding.tight (S.rarrow ^^ break 1) ~indent:3 in
      let args =
        List.mapi
          (fun i arg ->
            if i = 0
  ...
  and pp_desc ?preceeding ct =
    ...
    | Ptyp_quote ct ->
      let preceeding, pre_nest =
        Preceeding.(preceeding + spaced S.lt_lbracket)
      in
    ...
    | Ptyp_splice ct ->
      ...
      in
      let preceeding, _ =
        Preceeding.(preceeding + tight (S.dollar ^^ break spaces))
      in
      pp ~preceeding ct
    | Ptyp_of_kind jkind ->
    ...
    | Ptyp_extension ext ->
      Extension.pp ext |> Preceeding.group_with preceeding |> fst
    | Ptyp_parens ct -> pp_parens ?preceeding ct

  and pp_parens ?preceeding ct =
    let lparen, pre_nest = Preceeding.(preceeding + tight S.lparen) in
  ...
  and pp_variant
    ...
    let pp_fields bracket =
      ...
      | rf :: rfs ->
        ...
        let first_pre, _ = Preceeding.(preceeding + spaced init) in
        let pipe_pre = Preceeding.spaced S.pipe ~indent in
    ...
    | Closed, Some labels ->
      ...
      ^/^ pre_nest
            (S.gt ^/^ separate_map (break 1) pp_poly_tag labels ^/^ S.rbracket)
    | Open, Some _ -> assert false

  and row_field
    preceeding
    { prf_desc; prf_attributes; prf_doc; prf_loc = _; prf_tokens = _ }
    =
  ...
  and row_field_desc preceeding = function
    ...
    | Rtag (label, _, []) ->
      pp_poly_tag label.txt |> Preceeding.group_with (Some preceeding) |> fst
    | Rtag (label, has_const, at_types) ->
      let pre_tag, pre_nest =
        Preceeding.group_with
          (Some preceeding)
  ...
  and pp_tuple ?preceeding elts =
    let pp_elt ?preceeding (lbl_opt, ct) =
      ...
      | Some s ->
        let lbl, pre_nest =
          string s ^^ S.colon ^^ break 0 |> Preceeding.group_with preceeding
        in
        lbl ^^ pre_nest (pp ct)
    in
    let pre_nest = Preceeding.implied_nest preceeding in
    let elts =
      List.mapi
        (fun i elt ->
          if i = 0 then pp_elt ?preceeding elt else pre_nest (pp_elt elt))
        elts
    in
  ...
  and object_field
    { pof_desc; pof_attributes; pof_doc; pof_tokens; pof_loc = _ }
    =
    let semi = List.exists is_semi pof_tokens in
    prefix
      (group (object_field_desc pof_desc))
      (Attribute.pp_list pof_attributes ^?^ optional Doc.pp pof_doc)
  ...
  and pp_alias ?preceeding aliased alias jkind =
    ...
    let parens = if Option.is_none jkind then Fun.id else parens in
    let alias =
      match alias with
      | None -> pp_any jkind
...
and Pattern : sig
  ...
  and pp_desc ~preceeding pat_flatness p =
    ...
    | Ppat_any -> Preceeding.group_with preceeding S.underscore |> fst
    | Ppat_var name ->
      Preceeding.group_with preceeding (str_or_op name.txt) |> fst
    | Ppat_alias (p, alias) ->
      let pre_nest = Preceeding.implied_nest preceeding in
      pp ?preceeding p
      ^^ (* try to fit the alias at the end of the current line, but prioritize
         ...
         group
           (break 1
            ^^ pre_nest
            @@ nest 2 (group (S.as_ ^/^ str_or_op alias.txt)))
    | Ppat_constant c -> Preceeding.group_with preceeding (constant c) |> fst
    | Ppat_unboxed_unit ->
      Preceeding.group_with preceeding (S.hash_lparen ^^ S.rparen) |> fst
    | Ppat_unboxed_bool b ->
    ...
    | Ppat_cons (hd, tl) ->
      let pre_nest = Preceeding.implied_nest preceeding in
      group (pp_cons ?preceeding hd ^/^ pre_nest (pp_cons ~on_right:true tl))
  ...
  and pp_parens ~preceeding ~optional flatness pat =
    ...
    | _ ->
      ...
      in
      let before, pre_nest =
        (* we explicitely pass the indent as it is guessed incorrectly for
           vanishing tokens... *)
        Preceeding.(preceeding + tight before ~indent:1)
      in
  ...
  and pp_parens_tuple ~preceeding ~attrs ~optional ~closed flatness pats =
    ...
    in
    let lparen, pre_nest =
      (* same remark as in [pp_parens] regarding the explicit indent. *)
      Preceeding.(preceeding + tight lparen ~indent:2)
    in
    let comma = Preceeding.spaced S.comma in
  ...
  and pp_open ~preceeding lid p =
    let space =
      match p.ppat_desc with
      | Ppat_unboxed_tuple _ | Ppat_record_unboxed_product _ -> break 1
      | _ -> empty
    in
  ...
  and pp_delimited_seq ~preceeding (opn, cls) nb_semis = function
    ...
    | elts ->
      ...
      let pre_opn, pre_nest =
        (* Is [~indent:2] correct for arrays? *)
        Preceeding.(preceeding + spaced opn ~indent:2)
      in
      let semi_as_pre = Preceeding.spaced S.semi in
      let opn_and_pats =
        List.mapi
          (fun i pat ->
            if i = 0
  ...
  and pp_constraint ~preceeding p ty_opt modes =
    ...
    let p_with_extras =
      ...
      | Some ty ->
        ...
        let p =
          p
          ^^ pre_nest (group (break 1 ^^ S.colon) ^/^ nest 2 (Core_type.pp ty))
        in
  ...
  and pp_construct ~preceeding name arg_opt =
    ...
    match arg_opt with
    | None -> name
    | Some ([], arg_pat) -> name ^/^ pre_nest @@ nest 2 (pp arg_pat)
  ...
  and pp_record ~preceeding ?(unboxed = false) nb_semis closed_flag fields =
    ...
    let pre_lbrace, pre_nest =
      let lbrace = if unboxed then S.hash_lbrace else S.lbrace in
      Preceeding.(preceeding + spaced lbrace ~indent:2)
    in
    let semi_as_pre = Preceeding.spaced S.semi in
    group
      (foldli
...
and Expression : sig
  ...
  let pp_op op =
    ...
    | Pexp_ident { txt = { desc = Lident Op s; _ }; _ } -> string s
    | _ -> assert false
  ;;
  ...
  let rec needs_space_if_prefixed e =
    ...
    | Pexp_tuple ({ parg_desc = Parg_labelled _; _ } :: _) -> true
    ...
    | Pexp_parens { optional = true; exp = lhs } -> needs_space_if_prefixed lhs
    | _ -> false
  ;;

  let rec pp ?preceeding e =
    pp_desc ~preceeding e |> Attribute.attach ~attrs:e.pexp_attributes

  and opt_space_then_pp ~extra_nest e =
    extra_nest ((if needs_space_if_prefixed e then break 1 else empty) ^^ pp e)

  and pp_desc ~preceeding exp =
    let ( !! ) kw = Ext_attribute.decorate kw exp.pexp_ext_attr in
    match exp.pexp_desc with
    | Pexp_ident lid ->
      Preceeding.group_with preceeding (longident lid.txt) |> fst
    | Pexp_constant c -> Preceeding.group_with preceeding (constant c) |> fst
    | Pexp_unboxed_unit ->
      Preceeding.group_with preceeding (S.hash_lparen ^^ S.rparen) |> fst
    | Pexp_unboxed_bool b ->
      Preceeding.group_with preceeding (if b then S.hashtrue else S.hashfalse)
    ...
    | Pexp_idx (ba, uas) ->
      (* FIXME: indent! *)
      Preceeding.group_with preceeding (pp_block_idx ba uas) |> fst
    ...
    | Pexp_seq_empty e ->
      pp ?preceeding e
      ^^ S.semi
      (* the following is needed if we're at the end of a record (or override)
         field for instance.
    ...
    | Pexp_while (e1, e2) ->
      ...
      let fst_line = group (while_ ^/^ pre_nest (nest 2 (pp e1) ^/^ S.do_)) in
      fst_line
      ^^ hardline
    ...
    | Pexp_for (p, e1, e2, dir, e3) ->
      ...
      group fst_line
      ^^ hardline
      ^^ pre_nest (nest 2 (pp e3))
      ^^ hardline
      ^^ pre_nest S.done_
    | Pexp_constraint (e, ct_opt, modes) ->
      pp_constraint ~preceeding e ct_opt modes
    | Pexp_coerce (e, ct1, ct2) -> pp_coerce ~preceeding e ct1 ct2
    | Pexp_send (e, lbl) ->
      let pre_nest = Preceeding.implied_nest preceeding in
      pp ?preceeding e
      ^^ break 0
    ...
    | Pexp_setvar (lbl, e) ->
      ...
      in
      group (pre_lbl ^/^ pre_nest S.larrow) ^/^ pre_nest @@ nest 2 (pp e)
    ...
    | Pexp_letexception (ec, body) ->
      ...
      let binding =
        ...
        let_exn
        ^/^ pre_nest
              (nest 2 (Extension_constructor.pp pretend_flat ec) ^/^ S.in_)
      in
      binding ^^ hardline ^^ pre_nest (pp body)
    | Pexp_assert e ->
      let assert_, pre_nest = Preceeding.group_with preceeding !!S.assert_ in
    ...
    | Pexp_overwrite (e1, e2) ->
      ...
      in
      group (overwrite ^/^ pre_nest @@ nest 2 (pp e1))
      ^/^ group
    ...
    | Pexp_splice e ->
      ...
      let preceeding, _ =
        Preceeding.(preceeding + tight (S.dollar ^^ break space))
      in
      pp ~preceeding e
    ...
    | Pexp_begin_end exp ->
      ...
      begin_ ^?^ pre_nest (nest 2 (optional pp exp) ^?^ S.end_)
  ...
  and pp_let ~preceeding mf rf vbs body =
    ...
    let bindings =
      Value_binding.pp_list
        ...
        ~add_in:true
    in
    group bindings ^^ hardline ^^ pre_nest (pp body)
  ...
  and pp_function_parts ?preceeding exp =
    ...
    | Pexp_function (params, constr, body) ->
      ...
      let fun_and_params =
        ...
        fun_
        ^/^ pre_nest
        @@ nest 2 (group (params ^/^ group (constr ^?^ S.rarrow)))
      in
      let function_or_empty, cases_or_body = Function_body.pp_parts body in
      let fun_and_function =
        group fun_and_params ^?^ pre_nest function_or_empty
      in
      group fun_and_function, pre_nest cases_or_body
    | _ -> assert false
  ...
  and pp_try ~preceeding ~ext_attrs e cases =
    ...
    in
    let cases = Case.pp_list cases in
    group (try_ ^/^ pre_nest (nest 2 (pp e)) ^/^ pre_nest S.with_)
    ^^ hardline
    ^^ pre_nest cases
  ...
  and pp_index_op ~preceeding nb_semis kind seq op indices assign =
    let open_, close, spaces_around_indices =
      let has_quotation =
        (* Over-approximation: quotations are only problematic in the first and
           ...
           opening/closing is lexed). But ... whatever. *)
    ...
    | None -> group access
    | Some e ->
      let preceeding = Preceeding.spaced S.larrow in
  ...
  and pp_ite ?preceeding ?(kw = S.if_) ext_attr e1 e2 e3_o =
    let pre_kw, pre_nest =
      ...
      Preceeding.(preceeding + tight pre_ext)
    in
  ...
  and pp_else_branch = function
    | { pexp_ext_attr = ext_attrs
      ; pexp_attributes =
          No_attributes (* we'd need to be under parens to have attrs *)
    ...
    | elts ->
      ...
      let opn_and_elts =
        foldli
          (fun i acc elt ->
            if i = 0
            then pp ~preceeding:pre_opn elt
            else acc ^^ break 0 ^^ pre_nest @@ pp ~preceeding:semi_as_pre elt)
          empty
          elts
      in
      group
        (opn_and_elts
         ^^ pre_nest
              ((if semi_as_term then break 0 ^^ S.semi else empty) ^/^ cls))
  ...
  and pp_coerce ~preceeding e ct1 ct2 =
    ...
    let colon_ct1 =
      optional (Core_type.pp ~preceeding:(Preceeding.spaced S.colon)) ct1
    in
    let coerce_ct2 =
      Core_type.pp ~preceeding:(Preceeding.spaced S.coerce) ct2
    in
    group
      (lparen_e ^/^ pre_nest (nest 1 (colon_ct1 ^?^ coerce_ct2) ^^ S.rparen))
  ...
  and pp_override ~preceeding nb_semis fields =
    ...
    let fields =
      if semi_as_term
      then separate_map (break 1) (fun elt -> field elt ^^ S.semi) fields
      else separate_map (S.semi ^^ break 1) field fields
    in
    let lbrace_lt, pre_nest = Preceeding.group_with preceeding S.lbrace_lt in
    group (lbrace_lt ^?^ pre_nest (nest 2 fields ^?^ S.gt_rbrace))
  ...
  and pp_block_idx block_access unboxed_accesses =
    parens
      (Block_access.pp block_access
       ^^ separate_map
            (break 0)
            (fun (Uaccess_unboxed_field lid) -> S.dothash ^^ longident lid.txt)
            unboxed_accesses)
  ...
  and pp_tuple ?preceeding ?(comma_aligner = empty) ?pre_nest_override elts =
    let pre_nest =
      ...
      | Some nest -> nest
    in
    let comma = Preceeding.spaced (comma_aligner ^^ S.comma) in
    foldli
      (fun i acc exp ->
        if i = 0
        then Argument.pp_preceeded ?preceeding pp exp
        else
          acc
          ^^ break 0
          ^^ pre_nest
          @@ Argument.pp_preceeded ~preceeding:comma pp exp)
      empty
      elts
  ...
  and pp_parens_tuple ~preceeding ~attrs ~kind flatness elts =
    ...
    in
    let lparen, pre_nest =
      (* NB. indent is not guessed correctly by [Preceeding] when given optional
         tokens. *)
      let indent = if kind = `Unboxed then 3 else 2 in
      Preceeding.(preceeding + tight lparen ~indent)
    in
    let elts =
      pp_tuple
        ~preceeding:lparen
        ~pre_nest_override:pre_nest
        elts
        ?comma_aligner
    in
  ...
  and pp_parens ~preceeding ~optional exp =
    ...
    | _ ->
      ...
      let indent =
        ...
        | _ -> 1
      in
      let before, pre_nest = Preceeding.(preceeding + tight before ~indent) in
      pp ~preceeding:before exp ^^ pre_nest after
      ...
      | Some op ->
        ...
        | Pexp_let_open _ ->
          (* These are the last elements of the chain (otherwise they'd be under
             ...
             those case. *)
          group (op ^/^ pp arg)
        | _ -> pp ~preceeding:(Preceeding.spaced op) arg
    in
    match arg.pexp_desc with
    | Pexp_apply (f, args) when on_right ->
      ...
      (match op.pexp_desc with
       | Pexp_ident { txt = { desc = Lident Op ":="; _ }; _ } ->
         ...
         default (Some op)
       | _ ->
         (* N.B. the app is not under parentheses, that's why this is valid! (we
            are not dropping attributes or anything) *)
         let op = pp_op op in
         let app_indent =
           Requirement.to_int (requirement op) + 1 (* space *) + 2 (* indent *)
         in
         let f = pp ~preceeding:(Preceeding.spaced op) f in
         Application.pp ~indent:app_indent f args
         |> Attribute.attach ~attrs:arg.pexp_attributes)
    | Pexp_function _ when on_right ->
      let op = pp_op (Option.get op) in
      let op_pre = Preceeding.spaced op ~indent:0 (* ! *) in
      ...
      foldli
        (fun i acc field ->
          ...
          then
            acc
            ^?^ nest_first_field
                  (Record_field.pp ?preceeding:first_pre pp field)
          else
            acc
            ^^ break 0
            ^^ pre_nest
            @@ Record_field.pp ~preceeding:semi_as_pre pp field)
        first_part
        fields
    in
    group
      (first_part_and_fields
       ^/^ pre_nest ((if semi_as_term then S.semi else empty) ^?^ S.rbrace))
  ...
  and pp_field ?preceeding ?(unboxed = false) e lid =
    ...
    let space =
      match e.pexp_desc with
      | Pexp_constant { pconst_desc = Pconst_integer (_, _, None); _ } ->
        (* We need a space here otherwise this would lex as an invalid literal.
           *)
        nbsp
      | _ -> empty
    in
    group (pp ?preceeding e ^^ space ^^ dot ^^ pre_nest (longident lid.txt))

  and pp_setfield ?preceeding e1 lid e2 =
    let pre_nest = Preceeding.implied_nest preceeding in
    let access = pp_field ?preceeding e1 lid in
    let assignment = pp ~preceeding:(Preceeding.spaced S.larrow) e2 in
...
and Record_field : sig
  ...
  let pp ?preceeding pp_value rf =
    let pre_name, pre_nest =
      Preceeding.group_with preceeding (longident rf.field_name.txt)
    in
    let lhs =
      pre_name
      ^?^ pre_nest
      @@ nest 2 (optional (fun v -> group @@ Type_constraint.pp v) rf.typ)
    in
    group
    @@
    match rf.value with
    | None -> lhs
    | Some v -> group (lhs ^/^ S.equals) ^/^ pre_nest @@ nest 2 (pp_value v)
  ;;
end
...
and Application : sig
  ...
  let is_function p =
    ...
    | Pexp_parens { optional = false; exp = { pexp_desc = Pexp_function _; _ } }
      -> true
    | _ -> false
  ;;
  ...
  let pp_function_parts ?app_prefix_flatness ?lbl exp =
    ...
    | Pexp_parens { exp = fun_exp; optional = false } ->
      ...
      , group
          (nest
             ?vanish:(Option.map Condition.flat app_prefix_flatness)
             2
             (break 1 ^^ body)
           ^^ S.rparen)
  ...
  let pp_arg_parts ?app_prefix_flatness arg =
    ...
    | Parg_unlabelled
      ...
      when is_function arg -> pp_function_parts ?app_prefix_flatness arg
    | Parg_labelled
        { optional = false
        ; legacy_modes = No_modes
        ; name
        ; maybe_punned = Some arg
        ; typ_constraint = None
        ; modes = No_modes
        ; default = None
        }
    ...
    | _ -> Argument.pp Expression.pp arg, empty
  ;;
  ...
  let pp ?(extra_nest = Fun.id) ?(indent = 2) f args =
    ...
    group ~flatness:app_prefix_flatness (f ^/^ extra_nest @@ nest indent args)
...
and Case : sig
  ...
  let pp_guard = function
    | None -> empty
    | Some e -> Expression.pp ~preceeding:(Preceeding.spaced S.when_) e
  ;;
  ...
  let pp_pattern pipe p =
    let rec split_top_or p =
      ...
      | _ -> [ p ]
    in
    let pipe_pre = Some (Preceeding.spaced S.pipe) in
    let first_pre = if pipe then pipe_pre else None in
    split_top_or p
    |> List.fold_left
         (fun (acc, preceeding) pat ->
...
and Letop : sig
  ...
  let pp ?preceeding { let_; ands; body } =
    let pre_nest = Preceeding.implied_nest preceeding in
    Binding_op.pp_list ?preceeding (let_ :: ands)
    ^^ hardline
    ^^ pre_nest
    @@ Expression.pp body
  ;;
...
and Argument : sig
  ...
  let pp_generic
    ...
    =
    let typ_and_modes =
      optional Type_constraint.pp typ_constraint |> with_modes ~modes:at_modes
    in
    ...
    pre_arg_ty_modes
    ^?^ pre_nest
    @@ nest 2 (optional (fun d -> S.equals ^/^ Expression.pp d) default)
  ;;

  let pp pp_arg arg =
    let parenthesize doc = if had_parens arg then parens doc else doc in
    group
    @@
    match arg.parg_desc with
    | Parg_unlabelled
  ...
  let pp_preceeded
    ?preceeding
    (pp_preceeded_arg : ?preceeding:Preceeding.t -> _)
    arg
    =
    let parenthesize fst_tok =
      ...
      in
      let pre, pre_nest =
        Preceeding.(preceeding + tight (fst_tok ^^ break 0 ^^ lparen) ~indent)
      in
      pre, pre_nest rparen
    in
    group
...
and Function_param : sig
  ...
  let pp_desc = function
    ...
    | Pparam_newtype newtype -> parens (S.type_ ^/^ Newtype.pp newtype)
    | Pparam_newtypes lst ->
...
and Function_body : sig
  ...
  let pp_cases ?preceeding cases ext_attrs =
    ...
    in
    pre_function_, pre_nest (softest_line ^^ Case.pp_list cases)
  ;;

  let pp_parts ?preceeding fb =
    match fb.pfb_desc with
    | Pfunction_body e -> empty, Expression.pp ?preceeding e
    | Pfunction_cases (ext_attrs, cases) -> pp_cases ?preceeding cases ext_attrs
  ;;
...
and Type_constraint : sig
  ...
  let pp = function
    | Pconstraint ct -> S.colon ^/^ Core_type.pp ct
...
and Comprehension : sig
  ...
  let pp_clause = function
    | Pcomp_for l ->
      group
        (S.for_
         ^/^ separate_map (break 1 ^^ S.and_ ^^ break 1) pp_clause_binding l)
    | Pcomp_when e -> group (S.when_ ^/^ Expression.pp e)
  ;;
  ...
  let pp_expr ?preceeding e =
    let left, right, c =
      ...
      | Pcomp_array_comprehension (mut, c) ->
        ...
        left, right, c
    in
    let left_pre, pre_nest = Preceeding.(preceeding + spaced left ~indent:2) in
...
and Value_description : sig
  ...
  let pp_type flatness ct =
    ...
    | None -> Core_type.pp ~preceeding:(Preceeding.spaced S.colon) ct
    | Some (poly_params, args, ret) ->
  ...
  let pp
    ...
    group
      ~flatness
      (group (kw ^/^ str_or_op pval_name.txt)
       ^/^ nest
             2
             (with_modalities
                ~modalities:pval_modalities
...
and Record : sig
  ...
  let pp_field
    ...
    let typ_and_modalities =
      Core_type.pp pld_type |> with_modalities ~modalities:pld_modalities
    in
    group (fld_name_colon ^/^ pre_nest @@ nest 2 typ_and_modalities)
    ^?^ pre_nest
    @@ nest
         3 (* oxcamlformat indents by 3 instead of 2 here, why? *)
         (group
  ...
  let pp_decl ?(unboxed = false) lbls =
    let maybe_trailing_semi, lbls =
      List.fold_left_map
        (fun preceding_tok lbl ->
          let next_tok =
            if List.exists is_semi lbl.pld_tokens then S.semi else empty
          in
          let preceeding = Preceeding.spaced preceding_tok ~indent:2 in
...
and Constructor_arguments : sig
  ...
  let pp_one
    ...
    =
    (if pca_global then S.global__ else empty) ^?^ Core_type.pp pca_type
  ...
  let pp td_flatness tokens name vars args res attrs post_doc =
    ...
    let constr =
      ...
      | args, None ->
        ...
        ^/^ pre_nest
        @@ nest 2
        @@ Constructor_arguments.pp args
      | Pcstr_tuple [], Some ct ->
        group (pipe_and_name ^/^ pre_nest @@ nest 2 S.colon)
        ^?^ pre_nest
        @@ nest 2 (vars ^?^ Core_type.pp ct)
      | args, Some ct ->
        group (pipe_and_name ^/^ pre_nest @@ nest 2 S.colon)
        ^?^ pre_nest
        @@ nest
...
and Type_declaration : sig
  ...
  let rhs_parts ?(subst = false) td_flatness td =
    ...
    | Some ct, kind ->
      ...
      ; S.equals
      ; private_ td.ptype_private ^?^ type_kind td_flatness kind
      ]
    | None, kind ->
      [ eq; private_ td.ptype_private; type_kind td_flatness kind ]
  ;;
  ...
  let pp ~start ?subst td =
    ...
    in
    let tconstr =
      Type_constructor.pp_decl
        td.ptype_tokens
        td.ptype_params
        (string td.ptype_name.txt)
    in
    let jkind_parts =
    ...
    in
    prefix (group ~flatness without_constraint) (pp_constraints td.ptype_cstrs)
  ...
  let rec pp_list ?subst start = function
    | [] -> empty
    | td :: tds -> group (pp ?subst ~start td) ^?^ pp_list ?subst [ S.and_ ] tds
  ;;

  let pp_list ?subst rf tds = pp_list ?subst [ S.type_; nonrec_ rf ] tds
end
  ...
  let pp
    ...
    let tconstr =
      Type_constructor.pp_decl
        ptyext_tokens
        ptyext_params
        (longident ptyext_path.txt)
    in
    let cstrs =
      separate_map
        (break 1)
        (Extension_constructor.pp flatness)
        ptyext_constructors
    in
    flow
      (break 1)
...
and Class_type : sig
  ...
  let rec pp_desc ct =
    ...
    | Pcty_arrow (arrow_arg, rhs) ->
      ...
      Core_type.Arrow.pp
        (flatness_tracker () (* never flat? *) )
        []
        args
        ~pp_rhs:(fun ?preceeding () ->
          (* FIXME! *)
          Preceeding.group_with preceeding (pp rhs) |> fst)
  ...
  and pp_signature attrs { pcsig_self; pcsig_fields } =
    ...
    in
    prefix obj_with_self (separate_map (break 1) pp_field pcsig_fields)
    ^/^ S.end_
  ...
  and pp
    ({ pcty_desc = _; pcty_attributes; pcty_loc = _; pcty_tokens = _ } as ct)
    =
    pp_desc ct |> Attribute.attach ~attrs:pcty_attributes
  ;;
end
...
and Class_infos : sig
  ...
  let pp
    ...
    let mk_constraint ct =
      Single_part { equal_or_colon = S.colon; body = Class_type.pp ct }
    in
    pp
...
and Class_description : sig
  ...
  let pp_ct ct =
    Layout_module_binding.Single_part
      { equal_or_colon = S.colon; body = Class_type.pp ct }
  ;;

  let pp_list = Class_infos.pp_list pp_ct ~keywords:[ S.class_ ]
end
...
and Class_type_declaration : sig
  ...
  let pp_ct ct =
    Layout_module_binding.Single_part
      { equal_or_colon = S.equals; body = Class_type.pp ct }
  ;;

  let pp_list = Class_infos.pp_list pp_ct ~keywords:[ S.class_; S.type_ ]
end
...
and Class_expr : sig
  ...
  let rec pp_desc ext_attr d =
    ...
    | Pcl_fun (params, rhs) ->
      ...
      prefix (prefix !!S.fun_ (group (params ^/^ S.rarrow))) (pp rhs)
        ...
        ~start:
          (S.let_
           :: match rf with
              ...
              | Recursive -> [ S.rec_ ])
      ^/^ pp body
    | Pcl_constraint (ce, ct) -> parens (pp ce ^/^ S.colon ^/^ Class_type.pp ct)
  ...
  and pp_value ~attrs lbl mut = function
    ...
    | Cfk_concrete (over, vb) ->
      let start =
        [ Attribute.attach ~attrs (S.val_ ^^ override_ over); mutable_ mut ]
      in
  ...
  and pp_method ~attrs lbl priv = function
    ...
    | Cfk_concrete (over, vb) ->
      let start =
        [ Attribute.attach ~attrs (S.method_ ^^ override_ over); private_ priv ]
      in
      Value_binding.pp ~start vb ~add_in:false

  and pp
    { pcl_ext_attrs; pcl_desc; pcl_attributes; pcl_loc = _; pcl_tokens = _ }
    =
    pp_desc pcl_ext_attrs pcl_desc |> Attribute.attach ~attrs:pcl_attributes
  ;;
end
...
and Class_declaration : sig
  ...
  let pp_ce ce =
    Layout_module_binding.Single_part
      { equal_or_colon = S.equals; body = Class_expr.pp ce }
  ;;

  let pp_list = Class_infos.pp_list pp_ce ~keywords:[ S.class_ ]
end
...
and Module_type : sig
  ...
  let rec pp_desc = function
    ...
    | Pmty_functor (attrs, fps, mty, modes) ->
      ...
      ^/^ separate_map (break 1) Functor_parameter.pp fps
      ^/^ S.rarrow
      ^/^ with_modes ~modes (pp mty)
    | Pmty_functor_type (fps, mty, modes) ->
      separate_map (break 1) Functor_parameter.pp_type fps
      ^/^ S.rarrow
      ^/^ with_modes ~modes (pp mty)
    | Pmty_with (mty, cstrs) ->
      let flatness = flatness_tracker () in
    ...
    | Pmty_strengthen (mty, lid) -> pp mty ^/^ S.with_ ^/^ longident lid.txt
    | Pmty_parens mty -> parens (pp mty)

  and pp { pmty_desc; pmty_attributes; pmty_loc = _; pmty_tokens = _ } =
    pp_desc pmty_desc |> Attribute.attach ~attrs:pmty_attributes
  ;;

  let as_rhs
    ({ pmty_desc; pmty_attributes; pmty_loc = _; pmty_tokens = _ } as mty)
    ...
    | Pmty_typeof (attrs, me) ->
      ...
      (match Module_expr.as_rhs me with
       | Single_part t ->
         Single_part { equal_or_colon; body = prefix ^/^ nest 2 t.body }
       | Three_parts rhs ->
         Three_parts { rhs with equal_or_colon; start = prefix ^/^ rhs.start })
    | _ -> Single_part { equal_or_colon; body = pp mty }
  ;;
end
...
and Signature : sig
  ...
  let pp_item_desc = function
    ...
    | Psig_include (incl, modalities) ->
      with_modalities ~modalities (Include_description.pp incl) |> group
  ...
  let pp_interface sg =
    with_modalities empty ~modalities:sg.psg_modalities ^?^ pp_keeping_semi sg
  ;;
end
...
and Module_declaration : sig
    ...
    let params, rhs =
      ...
      | With_params (params, mty, modes) ->
        let rhs =
          ...
          |> Layout_module_binding.set_punct S.colon
          |> Layout_module_binding.map_rhs_end (with_modes ~modes)
        in
        List.map Functor_parameter.pp params, rhs
      | Without_params (mty, modalities) ->
        let rhs =
          Module_type.as_rhs mty
          |> Layout_module_binding.set_punct (equal_sign mty)
  ...
  let pp_recmods = function
    | [] -> empty (* assert false? *)
    | md :: mds ->
      pp [ S.module_; S.rec_ ] md ^?^ separate_map (break 1) (pp [ S.and_ ]) mds
  ;;
...
and Module_substitution : sig
  ...
  let pp
    ...
    let rhs =
      Layout_module_binding.Single_part
        { equal_or_colon = S.colon_equals; body = longident pms_manifest.txt }
    in
    Layout_module_binding.pp
      ~item:true
      ?pre_doc:pms_pre_doc
  ...
  let pp
    ...
    let rhs =
      Option.map
        (fun mty ->
          Module_type.as_rhs mty
          |> Layout_module_binding.set_punct
...
and Open_infos : sig
  ...
  let pp
    ...
    =
    let open_kw =
      Ext_attribute.decorate_optional_override
        S.open_
        popen_override
        popen_ext_attrs
    in
    ...
    let open_ =
      ...
      | Layout_module_binding.Single_part doc ->
        group (keywords ^/^ pre_nest @@ nest 2 doc.body)
      | Three_parts { start; main; stop; _ } ->
        group (keywords ^/^ pre_nest start) ^/^ pre_nest (nest 2 main ^/^ stop)
    in
...
and Open_description : sig
  ...
  let pp =
    Open_infos.pp (fun lid ->
      Single_part { equal_or_colon = empty; body = longident lid.txt })
  ;;
end
...
and Include_infos : sig
  ...
  let pp
    ...
    let include_ =
      ...
      | Layout_module_binding.Single_part doc ->
        group (keywords ^/^ nest 2 doc.body)
      | Three_parts { start; main; stop; _ } ->
        group (keywords ^/^ start) ^/^ nest 2 main ^/^ stop
    in
...
and With_constraint : sig
  ...
  let pp wc =
    ...
    | Pwith_type (params, lid, priv, ct, cstrs) ->
      S.type_
      ^/^ Type_constructor.pp_decl wc.wc_tokens params (longident lid.txt)
      ^/^ S.equals
      ^?^ private_ priv
      ^?^ Core_type.pp ct
      ^?^ Type_declaration.pp_constraints cstrs
    ...
    | Pwith_modtype (lid, mty) ->
      ...
      ^/^ Module_type.pp mty
    | Pwith_jkind (lid, jk) ->
      S.kind__ ^/^ longident lid.txt ^/^ S.equals ^/^ Jkind_annotation.pp jk
    ...
    | Pwith_modsubst (lid1, lid2) ->
      S.module_ ^/^ longident lid1.txt ^/^ S.colon_equals ^/^ longident lid2.txt
...
and Module_expr : sig
  ...
  let rec pp { pmod_desc; pmod_attributes; pmod_loc = _; pmod_tokens = _ } =
    pp_desc pmod_desc |> Attribute.attach ~attrs:pmod_attributes
  ...
  let as_rhs
    ...
    : Layout_module_binding.rhs
    =
    let equal_or_colon = S.equals in
...
and Structure : sig
  ...
  let pp_item item =
    ...
    | Pstr_extension te -> Extension.pp_toplevel te |> group
  ...
  let pp_implementation str = group (pp_keeping_semi str)
end
      ...
      group
        (S.colon
         ^/^ nest
               2
               (group
                  (S.type_ ^/^ separate_map (break 1) Newtype.pp vars ^^ S.dot)))
...
and Layout_module_binding : sig
  ...
  val set_punct : t (* equal or colon! *) -> rhs -> rhs
  ...
  let set_punct equal_or_colon = function
    | Single_part r -> Single_part { r with equal_or_colon }
    | Three_parts r -> Three_parts { r with equal_or_colon }
  ;;
  ...
  let pp
    ...
    bound
    =
    let keyword, pre_nest = Preceeding.group_with preceeding (group keyword) in
    let bindings =
      ...
      | _ ->
        group
          (main
           ^/^ separate_map
                 (break 1)
                 (fun p -> pre_nest @@ nest params_indent p)
                 params)
    in
    let pre_nest_2 x = pre_nest (nest 2 x) in
    (* In the following, we try to keep colons (or sometimes @@) and equals at
       end of line. *)
    match constraint_, rhs with
    | None, None -> (* abstract module types *) bindings
    | None, Some Single_part { equal_or_colon; body }
    | Some Single_part { equal_or_colon; body }, None ->
      (* [module A = B], [module M : S], etc. *)
      group
      @@ flow (break 1) [ bindings; pre_nest_2 equal_or_colon; pre_nest_2 body ]
    | ( Some Single_part { equal_or_colon = colon; body = typ }
      , Some Single_part { equal_or_colon = equal; body = exp } ) ->
      (* [module M : S = N],
         {[
           module M : S with type foo = bar with module M = N = MODULE_EXPR
         ]}
         ,
         {[
           module M : (functor () -> MODULE_TYPE) = functor () -> MODULE_EXPR
         ]} *)
      group
      @@ flow
           (break 1)
    ...
    | Some Three_parts { equal_or_colon; start; main; stop }, None ->
      ...
      group
        (flow
           ...
           ; pre_nest
               start (* don't indent struct/sig if it's on its own line *)
           ]
         ^?^ pre_nest_2 main
         ^?^ pre_nest stop)
    | ( Some Three_parts { equal_or_colon = colon; start; main; stop }
      ...
      group
        (flow
         ...
         ^?^ pre_nest_2 main
         ^?^ flow (break 1) [ pre_nest stop; pre_nest_2 equal; pre_nest_2 body ])
    | ( Some Single_part { equal_or_colon = colon; body = typ }
  ...
  let pp
    ...
    (* Here we assume that [preceeding] cannot be [Some _] at the same time as
       [pre_text] or [pre_doc]. *)
    pp ?preceeding ?params_indent ~keyword ~params ?constraint_ ?rhs bound
...
and Value_binding : sig
  ...
  let layout ?preceeding ~keyword ~params ?constraint_ ?rhs bound in_kw =
    ...
    let bindings =
      ...
      | _ ->
        group
          (main
           ^/^ separate_map (break 1) (fun p -> pre_nest @@ nest 2 p) params)
    in
    match constraint_, rhs with
    (* let-punned *)
    ...
    | None, Some Layout_module_binding.Single_part { equal_or_colon; body } ->
      group
        (group (bindings ^/^ pre_nest @@ nest 2 equal_or_colon)
         ^/^ pre_nest (nest 2 body)
         ^?^ pre_nest in_kw)
    | None, Some Three_parts { equal_or_colon = equal; start; main; stop } ->
      let bindings_and_main =
        group (bindings ^/^ pre_nest @@ nest 2 (group (equal ^/^ start)))
        ^/^ pre_nest
        @@ nest 2 main
      in
      group (group bindings_and_main ^?^ pre_nest (stop ^?^ in_kw))
    (* constraint and body *)
    | Some typ, Some Single_part { equal_or_colon = equal; body = exp } ->
      group
        (group
      ...
      let bindings_cstr_main =
        group
          (group (bindings ^/^ pre_nest @@ nest 2 doc)
           ^/^ pre_nest (nest 2 @@ group (equal ^/^ start)))
        ^/^ pre_nest
        @@ nest 2 main
      in
      group (group bindings_cstr_main ^?^ pre_nest stop ^?^ pre_nest in_kw)
  ;;
  ...
  let rhs e =
    ...
    | Pexp_function _ ->
      ...
      let body = Attribute.attach fun_ ~attrs:e.pexp_attributes in
      Single_part { equal_or_colon; body }
  ...
  let pp
    ...
    let constraint_ =
      ...
      | Some vc, lst -> Some (with_modes (Value_constraint.pp vc) ~modes:lst)
    in
    let attrs, in_kw =
      if not add_in
      then pvb_attributes, empty
      else
        (* The attrs go before [in]. In this case there are also no docs, so the
           later call to attach is a noop. But I'd rather leave it than have two
           separate code paths depending on whether we're adding [in] or not. *)
...
and Module_binding : sig
  ...
  let modal_constraint constr mode_l : Layout_module_binding.rhs option =
    let mty_as_rhs mty =
      Module_type.as_rhs mty |> Layout_module_binding.set_punct S.colon
    in
    match Option.map mty_as_rhs constr, mode_l with
    | None, No_modes -> None
    | Some rhs, modes ->
      Some (Layout_module_binding.map_rhs_end (with_modes ~modes) rhs)
    | None, at_modes ->
...
and Jkind_annotation : sig
  ...
  let rec pp_desc = function
    ...
    | Pjk_operator (jk, axes) ->
      flow (break 1) (pp jk :: List.map (fun a -> string a.Location.txt) axes)
    | Pjk_mod (jk, ms) -> pp jk ^/^ S.mod_ ^/^ modes ms
    | Pjk_with (jk, ct, modalities) ->
      pp jk
      ^/^ group
            (S.with_ ^/^ nest 2 (Core_type.pp ct |> with_modalities ~modalities))
    | Pjk_kind_of ct -> S.kind_of__ ^/^ Core_type.pp ct
    | Pjk_product jks -> separate_map (break 1 ^^ S.ampersand ^^ break 1) pp jks
...
and Jkind_declaration : sig
  ...
  let pp
    ...
    Attribute.attach
      ~item:true
      ~attrs:pjkind_attributes
      (kw
       ^/^ string pjkind_name.txt
       ^?^
...
(* (**
   ...
   *)
   ...
   type toplevel_phrase = | Ptop_def of structure | Ptop_dir of
   toplevel_directive (** [#use], [#load] ... *)

   and toplevel_directive =
   [{ pdir_name: string loc; pdir_arg: directive_argument option; pdir_loc: Location.t; }]

   and directive_argument =
   [{ pdira_desc: directive_argument_desc; pdira_loc: Location.t; }]

   and directive_argument_desc = | Pdir_string of string | Pdir_int of string *
   char option | Pdir_ident of Longident.t | Pdir_bool of bool *)
