...
(* Pretty-printing precedence.
   ...
   1. [Level] defines a totally-ordered scale of precedences from loosest to
      tightest: mandatory = -1 (parens are mandatory around a node at this
      level) free = 23 (parens are never needed; the node is never ambiguous)
      ...
      COLONCOLON, unary +/-, pattern as/|). The rest (function application,
      field access, atoms, attributed nodes, ...) use local lowercase_underscore
      names. Several may share one integer because they belong to unrelated
      grammar categories that are never compared against each other.

   2. [Expr.get_prec] / [Pattern.get_prec] / [Type.get_prec] return the tightest
      level at which a node [n] is still unambiguous when printed bare (i.e.
      without surrounding parens). This is an *upper bound* on the set of "safe"
      required levels:
   ...
   a required level (the lower bound: "the child must be unambiguous at *at
   least* this tightness") into each child position. After simplifying a child,
   the invariant is checked:

   required_level ≤ get_prec child

   When it holds, parentheses can be removed, the child is printed bare. When it
   fails the parens are preserved around that child. The parent's own level is
   passed to its children, possibly adjusted, so that the check is local to each
   edge of the tree.
...
module Level : sig
  ...
  val free
    :
      level
      (* unconstrained tail position: let body, match/try scrutinee, arrow
         codomain *)

  val below_semi
    :
      level
      (* below_SEMI: dangling-construct cap, bare-arg constructor prec,
         or-pattern / arrow-domain threshold *)
  ...
  val cons_head
    : level (* COLONCOLON head threshold, stricter than [cons] (right-assoc) *)

  val unary_sign : level (* prec_unary_minus / prec_unary_plus *)

  val attributed
    : level (* node carries [@attr]; forced here regardless of shape *)

  val apply
    : level (* application / method-send / prefix-keyword outward prec *)

  val apply_arg
    :
      level
      (* threshold required of an application / constructor / prefix-keyword
         argument *)
  ...
  val lazy_arg
    :
      level
      (* [lazy] / [exception] pattern argument threshold (same int as
         [field_access], unrelated) *)
  ...
  (* Infix-operator groups, mirroring parser.mly's %left/%right table. Each pair
     is (lhs_threshold, rhs_threshold): %left → (prec, prec+1) rhs rejects a
     bare same-level chain %right → (prec+1, prec) lhs rejects it *)
  val pow_op : level * level (* INFIXOP4 / ** lsl lsr asr *)
  val and_op : level * level (* AMPERSAND / AMPERAMPER *)
  val or_op : level * level (* OR / BARBAR *)

  val assign_op
    : level * level (* LESSMINUS / COLONEQUAL, approximated as one group *)

  val mul_op : level * level (* PERCENT / INFIXOP3 / MOD / STAR *)

  val plus_op
    : level * level (* INFIXOP2 / PLUS / MINUS / PLUSDOT / MINUSDOT / PLUSEQ *)
...
(* Patterns. ================================================================
   ...
   0 Ppat_alias, Ppat_or 3 Ppat_tuple 11 Ppat_cons 18 Ppat_lazy, Ppat_exception
   19 Ppat_construct (_, Some _), Ppat_variant (_, Some _) 22 everything
   self-delimited

   [Ppat_alias] has no single correct level: it binds tighter than [,] and
   [::]-head (parens there are redundant) but looser than [|], [::]-tail, and
   constructor arguments (parens there are necessary). It is conservatively
   treated as [free] everywhere, keeping a few redundant parens rather than risk
   a missing one. *)
module Pattern = struct
  ...
  let get_prec (p : pattern) : level =
    ...
    | No_attributes ->
      ...
      | Ppat_open _
      | Ppat_parens _
      | Ppat_constraint (_, _, _) -> atom
      | Ppat_construct (_, Some _) | Ppat_variant (_, Some _) -> apply_arg
      | Ppat_lazy _ | Ppat_exception _ -> apply
      | Ppat_cons _ -> cons
      | Ppat_tuple _ -> comma
  ...
  let rec simp_pat
    super
    (self : 'a Traversals.map_with_context)
    (ctx : 'a)
    (prec : level)
    (p : pattern)
    : pattern
    ...
    | Ppat_parens { pat; optional } ->
      ...
      let skip_parens =
        ...
        | No_attributes, No_attributes ->
          is_compatible (get_prec pat) ~parent:prec
    ...
    | Ppat_cons (p1, p2) ->
      (* Right-associative, like [Pexp_cons]: ([(a :: b) :: c] is different from
         [a :: b :: c]). The head must reject a bare cons The tail may accept
         one. *)
    ...
    | Ppat_lazy pat ->
      (* Stricter than [Ppat_construct]: `lazy A n` is a parse error where `S S
         N` is not, so the threshold must be above [apply_arg]. *)
  ...
  and simp_pat_arg
    ...
    | Parg_unlabelled _ | Parg_labelled _ -> self#argument self#pattern ctx x
  ;;
  (* TODO: parentheses around an argument are inlined in the token stream, with
     no explicit representation in the CST. This traversal should be refined to
     account for them and remove them when they are proven unnecessary. *)

  let simplify_pat super self ctx p = simp_pat super self ctx free p

  (* A function-parameter is a pattern with a strict threshold ([atom]):
     [let swap (a, b) = ...] and [let force_int (lazy n) = ...] cannot drop
     their parens.
  ...
  let simp_param super self ctx : function_param -> function_param =
    ...
    | Pparam_val arg ->
      let has_parens =
        Utils.token_exists arg.parg_tokens ~f:(function
          | LPAREN | RPAREN -> true
          | _ -> false)
      in
      let level = if has_parens then free else atom in
      { fp with
        pparam_desc = Pparam_val (simp_pat_arg super self ctx level arg)
      }
...
(* Core types. ==============================================================
   ...
   0 Ptyp_arrow 3 Ptyp_tuple 19 Ptyp_constr / Ptyp_class WITH arguments 22
   everything self-delimited

   Tuple types are flat/N-ary (same as expressions and patterns). [Ptyp_tuple]'s
   low outward prec means it is never stripped generically; it is only accepted
   where a low threshold is already correct (arrow domain, [as]).
...
module Type = struct
  ...
  let get_prec (t : core_type) : level =
    ...
    | No_attributes ->
      ...
      | Ptyp_of_kind _
      | Ptyp_parens _
      | Ptyp_unboxed_tuple _ -> atom
      | Ptyp_constr (_ :: _, _) | Ptyp_class (_ :: _, _) -> apply_arg
      | Ptyp_tuple elts ->
        (* Parentheses are mandatory around labelled tuples *)
        if List.exists (fun (lbl, _) -> Option.is_some lbl) elts
        then mandatory
        else comma
      | Ptyp_arrow _ -> free
  ...
  let rec simp_typ super ctx (prec : level) (t : core_type) : core_type =
    ...
    | Ptyp_arrow { domain; codom_legacy_modes; codom_type; codom_modes } ->
      ...
      let domain =
        { domain with aa_type = simp_typ below_semi domain.aa_type }
      in
...
module Expr = struct
  ...
  let rec get_prec (e : expression) : level =
    ...
    | No_attributes ->
      ...
      (* Dangling constructs (if/;, let/match/try/function/local-module/...)
         ...
         [below_semi] and further limited by their trailing sub-expression. This
         is safe in "any expression" positions but NOT as an application
         argument or infix left operand — those are handled by [rightmost] in
         [simp_expr]. *)
      | Pexp_ifthenelse (_, e, None)
      | Pexp_ifthenelse (_, _, Some e)
      | Pexp_sequence (_, e) -> get_stricter (get_prec e) below_semi
      ...
      (* Tuples: a bare [a, b] often means something different from [(a, b)]. It
         is always safe to keep parens. Jane Street-style favors parenthesizing
         tuples on multiple lines Since we don't know yet if tuple will get
         broken over multiple line, we always insert parentheses. They might get
      ...
      | Pexp_override _
      | Pexp_object _
      | Pexp_dot_open (_, _) -> mandatory
  ;;

  (* Would a bare `| pat -> ...` immediately following [e] be swallowed as
     another case of e's innermost match/try/function? Non-delimited
     right-extending constructions inherit the danger from their tail: let/in,
     if/then/else, unary/binary operators, fun x -> body, ... *)
  let rec ends_in_bare_cases (e : expression) : bool =
    ...
    | Pexp_letexception (_, body)
    | Pexp_let_open (_, body)
    | Pexp_letop { body; _ } -> ends_in_bare_cases body
    | Pexp_ifthenelse (_, e, None)
    | Pexp_ifthenelse (_, _, Some e)
  ...
  (* Would [e], printed bare, swallow the following [;] as part of its own body?
  ...
  (* Does [e] resolve to a bare [;] sequence? (possibly through a trailing
     position). Useful because [;] binds looser than [:=]/[<-]: [r := a; b] is
     [(r := a); b]. *)
  ...
  (* Is nothing textually following this position that a dangling construct
     could swallow? [is_dangling_tail] answers this for a specific [e], matching
     [get_prec]'s [below_semi] cap but EXCLUDING bare sequences (handled
     separately by [is_bare_sequence]). [Rightmost_no_fun_cases]: a bare
     `function | ...` is a hard parse error as unary +/-'s operand, unlike every
     other rightmost position. *)
  type rightmost =
    | Not_rightmost
    | Rightmost
    | Rightmost_no_fun_cases
  ...
  let simplify
    ...
    (* [rightmost] must be PROPAGATED, never hardcoded: a node is only truly
       rightmost if every step from where it was first established down to that
       node was tail-preserving. *)
    let rec simp_expr
      ...
      | Pexp_parens { exp; optional } ->
        ...
        let skip_parens =
          ...
          | Attributes _ -> false
        in
        if skip_parens then exp else return (Pexp_parens { exp; optional })
      ...
      | Pexp_add_or_sub (op, exp) ->
        (* Unary +/- operand is rightmost (minus bare `function`), never a bare
           `;` sequence: `- a; b` is `(- a); b`. *)
        let exp =
          simp_expr
            ~allow_bare_cases
            ~allow_semicolon_swallowers
            ~allow_bare_sequence:false
            ~rightmost:(no_fun_cases rightmost)
            unary_sign
            exp
        in
      ...
      | Pexp_tuple args ->
        (* Only the LAST element is rightmost; never a bare `;` sequence (`a, b;
           c` is `(a, b); c`). *)
      ...
      | Pexp_ifthenelse (cond, ethen, None) ->
        ...
        let ethen =
          simp_expr ~allow_bare_cases ~rightmost (fst assign_op) ethen
        in
      ...
      | Pexp_ifthenelse (cond, ethen, Some eelse) ->
        ...
        let eelse =
          simp_expr ~allow_bare_cases ~rightmost (fst assign_op) eelse
        in
      ...
      | Pexp_cons (exp1, exp2) ->
        (* Right-assoc: head must reject a bare cons; tail accepts one. Tail is
           rightmost, never a bare `;` sequence. *)
      ...
      | Pexp_array (mutf, elts) ->
        let elts =
          List.map (simp_expr ~allow_semicolon_swallowers:false free) elts
        in
      ...
      | Pexp_quote _
      | Pexp_hole
      | Pexp_comprehension _ -> super ctx e
    ...
    and simp_arg
      ...
      | Parg_labelled
        ...
        { x with
          parg_desc = Parg_labelled { parg with maybe_punned = Some arg }
        }
      | _ -> self#argument self#expression ctx x
    and simp_cases ?(rightmost = Not_rightmost) prec = function
      | [ x ] ->
        (* Last case body: inherits the outer threshold, is rightmost, nothing
           follows to swallow. *)
...
let remover =
  object(self)
    ...
    method! pattern ctx pat = Pattern.simplify_pat super#pattern self ctx pat
    ...
    method! value_binding ctx vb =
      ...
      (* With a type constraint, the pattern is NOT a free position: `let a, b :
         t = e` attaches the constraint to `b` alone. *)
      let pvb_pat =
      ...
      { vb' with
        pvb_pat
      ; pvb_expr
      ; pvb_params = List.map simp_param vb.pvb_params
      }
    ...
    method! core_type ctx t = Type.simplify_typ super#core_type ctx t
  end
;;
