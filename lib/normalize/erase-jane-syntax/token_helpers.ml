...
module Attributes : sig
  ...
  let mk_empty_payload ~attr_loc ~name_loc (name, name_tokens) =
    ...
    Ast_helper.Attr.mk
      ~loc:attr_loc
      ~tokens
      (Location.mkloc [ name ] name_loc)
      (PStr empty_payload)
  ;;

  let add parent_tokens parent_attrs attrs =
    match parent_attrs, attrs with
    | Attributes _, _ | _, No_attributes -> assert false
  ...
  let merge parent_tokens attrs1 attrs2 =
    ...
    | No_attributes, attrs | attrs, No_attributes -> parent_tokens, attrs
...
module Unboxed = struct
  ...
  (* A single CST node [P..._unboxed_tuple _] is transformed into two nested CST
     nodes: [P...parens (P..._tuple _)].
    ...
    | before_hlp, hlp :: after_hlp ->
      ...
      let outer_tokens =
        ...
        @ { hlp with desc = Token (LPAREN, false) }
          :: { desc = Child_node; pos = loc.Location.loc_start }
          :: lp_and_after
      in
  ...
  (* A single CST node [P..._unboxed_unit] is transformed into two nested CST
     nodes: [P...construct (Lident "()", None)].
  ...
  let unit ~loc tokens =
    let tokens, lid_tokens =
      ...
      | before, lparen :: rem ->
        (match split ~on:RPAREN rem with
         | between, rparen :: after ->
           ( before @ { desc = Child_node; pos = lparen.pos } :: after
           , { lparen with desc = Token (LPAREN, false) } :: between
             @ [ rparen ] )
         | _ -> assert false)
      | _ -> assert false
    in
  ...
  (* Likewise: one CST node/token sequence into two. But slightly simpler
     because there can't be comments. *)
  let bool ~loc tokens b =
    let unboxed, boxed, name =
      let open Parser_tokens in
      if b then HASHTRUE, TRUE, "true" else HASHFALSE, FALSE, "false"
    in
    let tokens, lid_tokens =
