...
module Ordering = struct
  ...
  let pp_error ppf : error -> unit = function
    | `Incomplete_flattening pos ->
      Format.fprintf
        ppf
        "@[<hov>@[<h>File %s, line %d,@ column %d:@]@ Incomplete \
         retokenisation of CST.@."
        pos.pos_fname
        pos.pos_lnum
        (column pos)
    | `Reordered (pos, t1, t2) ->
      Format.fprintf
        ppf
        "@[<hov>@[<h>File %s, line %d,@ column %d:@]@ Tokens %a and %a have \
         been swapped.@."
        pos.pos_fname
        pos.pos_lnum
        (column pos)
  ...
  let rec ensure_preserved : Tokens.seq -> _ = function
    | [] | [ _ ] -> Ok ()
    | { desc = Child_node; pos } :: _ -> Error (`Incomplete_flattening pos)
...
module Comments_comparison = struct
  ...
  let same_number before after =
    ...
    if nb_before = nb_after then Ok () else (Error `Comments_dropped)
  ;;
end
