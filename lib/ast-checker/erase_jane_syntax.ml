...
let constant_desc = function
  | Pconst_unboxed_integer (lit, modif) -> Pconst_integer (lit, Some modif)
  | Pconst_unboxed_float (lit, modif) -> Pconst_float (lit, modif)
  | Pconst_untagged_char c -> Pconst_char c
  | c -> c
;;
...
let is_exclave e =
  ...
  | Pexp_extension ({ txt = "extension.exclave"; loc = _ }, PStr []) -> true
  | _ -> false
;;
...
let lid_of_parts ~loc ident projs =
  ...
  List.fold_left
    (fun lid proj -> locate @@ Longident.Ldot (lid, locate proj))
    (locate @@ Longident.Lident ident)
    projs
;;
...
let rec expression e =
  ...
  in
  match e.pexp_desc with
  | Pexp_stack child | Pexp_borrow child -> fold_into ~parent:e child
  | Pexp_apply (maybe_exclave, [ Nolabel, child ]) when is_exclave maybe_exclave
  ...
  | Pexp_unboxed_tuple fields -> { e with pexp_desc = Pexp_tuple fields }
  | Pexp_record_unboxed_product (re, fields) ->
    { e with pexp_desc = Pexp_record (re, fields) }
  | Pexp_unboxed_field (re, fn) -> { e with pexp_desc = Pexp_field (re, fn) }
...
let core_type ct =
  ...
  | Ptyp_arrow (lbl, dom, codom, dms, cdms) ->
    let attrs =
      List.filter
        (fun a -> a.attr_name.txt <> "extension.curry")
        ct.ptyp_attributes
    in
    let ptyp_desc =
      match lbl with
  ...
  | Ptyp_unboxed_tuple cts -> { ct with ptyp_desc = Ptyp_tuple cts }
  | Ptyp_any Some _ -> { ct with ptyp_desc = Ptyp_any None }
  | Ptyp_var (name, Some _) -> { ct with ptyp_desc = Ptyp_var (name, None) }
...
let jkind_to_attr jk =
  ...
  | Pjk_abbreviation { txt = Lident ("immediate" | "immediate64" as s); loc } ->
