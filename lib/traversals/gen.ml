...
let to_modname fn =
  Filename.basename fn |> Filename.chop_extension |> String.capitalize_ascii
;;
...
let extract_type_decls fn =
  In_channel.with_open_text fn
  @@ fun ic ->
  let lb = Lexing.from_channel ic in
  let tds =
    if Filename.check_suffix fn ".mli"
    then Parse.interface lb |> List.filter_map from_sig_item
    else Parse.implementation lb |> List.filter_map from_str_item
  in
...
let traversals =
  ...
  let lift_class =
    ...
    then []
      ...
      [ Builder.pstr_class
          [ gen_class
              ~what:Ppxlib_traverse.Backends.lifter
              ~loc:Location.none
              tds
          ]
      ]
...
let () = Pprintast.structure Format.std_formatter mapper
