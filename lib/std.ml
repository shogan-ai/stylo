...
let read_input =
  ...
  let rec read_entry ic chunks =
    ...
    if nb_read = 0
    then if chunks = [] then None else Some (reconstruct chunks)
    else
      match Bytes.split_on_char '\000' (Bytes.sub buffer 0 nb_read) with
      | [] -> assert false
      | [ last ] ->
        (* no trailing '\000': unterminated final entry; the next read is EOF,
           which returns the accumulated chunks *)
