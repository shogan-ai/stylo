...
let mkclass ~loc ?ext_attrs ?attrs d =
  let tokens = Tokens.at loc in
  Cl.mk ~loc:(make_loc loc) ?ext_attrs ?attrs ~tokens d
let mkcty ~loc ?attrs d =
  let tokens = Tokens.at loc in
  Cty.mk ~loc:(make_loc loc) ?attrs ~tokens d
let mkconst ~loc c =
  let tokens = Tokens.at loc in
  Const.mk ~loc:(make_loc loc) ~tokens c
...
let mkexp_constraint ~loc ~exp ~cty ~modes =
  match cty, modes with
  | None, No_modes -> invalid_arg "empty mkexp_constraint" (* ? *)
...
let lapply ~loc p1 loc_p1 p2 loc_p2 =
  mklid ~loc (Lapply(mkrhs p1 loc_p1, mkrhs p2 loc_p2))

let pexp_extension ~loc ~id e =
  let tokens = Tokens.at loc in
  let str =
    { pst_items = [mkstrexp loc e No_attributes];
      pst_loc = make_loc loc;
      pst_tokens = tokens }
  in
  Pexp_extension (mkrhs id loc, PStr str, tokens)
...
let empty_body_constraint =
  { ret_type_constraint = None; ret_mode_annotations = No_modes}
...
let attributes_patching sloc prev_tokens prev_attributes attr =
  (* Called from rules of the form:
     {[
       elt = some_syntactic_element attr = attribute
     ]}
     where [attr] is going to be appended to [elt]'s attributes.

     We start by getting the tokens at that location, which will be of the form
     {[
       CMTS* Child_node CMTS* Child_node CMTS*
     ]}
     since [elt] and [attr] have both been reduced already.
  *)
  let tokens = Tokens.at sloc in
  ...
  (* There is one subtlety however: if [elt] didn't have any attributes yet, we
     ...
     attributes. *)
  match prev_attributes with
  | No_attributes ->
    let attr_startp = (List.hd attr_tokens).pos in
    let child_for_attrs = { Tokens.pos = attr_startp; desc = Child_node } in
    let elt_tokens = to_prepend @ prev_tokens @ [child_for_attrs] in
    let attrs =
      let attr_loc = make_loc (attr_startp, snd sloc) in
      Attributes { attributes = [attr]; loc = attr_loc; tokens = attr_tokens }
    in
    elt_tokens, attrs
  | Attributes { attributes; loc; tokens } ->
    (* There were already attributes, so we just need to append.
       We also take the opportunity to grow the location. *)
    let attrs =
      let attributes = attributes @ [attr] in
      let loc = { loc with loc_end = snd sloc } in
      let tokens = tokens @ attr_tokens in
      Attributes { attributes; loc; tokens }
    in
    to_prepend @ prev_tokens, attrs
...
%type <attributes> post_item_attributes
/* END AVOID */
...
module_expr:
  ...
  | me = module_expr attr = attribute
      { (* We are "patching" the module expr to add an attribute, we must also
        ...
        let new_tokens, attributes =
          attributes_patching $sloc me.pmod_tokens me.pmod_attributes attr
        in
        { me with pmod_tokens = new_tokens; pmod_attributes = attributes } }
  | mkmod(
      (* A module identifier. *)
...
%inline module_binding:
  ...
  name = module_name_modal(at_mode_expr, no_modes)
...
module_type:
  ...
  | mty = module_type attr = attribute
      { (* We are "patching" the module type to add an attribute, we must also
           ...
           relative to the attribute. *)
        let new_tokens, attributes =
          attributes_patching $sloc mty.pmty_tokens mty.pmty_attributes attr
        in
        { mty with pmty_tokens = new_tokens; pmty_attributes = attributes } }
  | mkmty(
...
%inline module_declaration:
  MODULE
  ext_attrs = ext_attributes
  name = module_name_modal(atat_modalities_expr, no_modalities)
...
class_expr:
  ...
  | ce = class_expr attribute
      { let new_tokens, attributes =
          attributes_patching $sloc ce.pcl_tokens ce.pcl_attributes $2
        in
        { ce with pcl_tokens = new_tokens; pcl_attributes = attributes } }
  | mkclass(
...
class_self_pattern:
  ...
  | LPAREN pattern COLON core_type RPAREN
      { mkpat_with_modes ~loc:$sloc ~pat:$2 ~cty:(Some $4) ~modes:No_modes }
  | /* empty */
      ...
      { let pat = mkpat ~loc:$loc($4) (Ppat_var (wrap_str $4)) in
        let vb =
          { pvb_pre_text = [];
            ...
            pvb_is_poly = false;
            pvb_pat = pat;
            pvb_legacy_modes = No_modes; pvb_modes = No_modes; pvb_params = [];
            pvb_constraint = None; pvb_ret_modes = No_modes;
            pvb_expr = Some $6; pvb_loc = make_loc $sloc;
            pvb_attributes = No_attributes;
            pvb_post_doc = None;
            pvb_tokens = Tokens.at $sloc; }
        in
        ($4, $3, Cfk_concrete ($1, vb)), No_attributes }
    ...
    EQUAL seq_expr
      {
        ...
        let vb =
          { pvb_pre_text = [];
            ...
            pvb_pat = pat;
            pvb_legacy_modes = No_modes; pvb_modes = No_modes; pvb_params = [];
            pvb_constraint = Some t; pvb_ret_modes = No_modes;
      ...
      { let params, tc, ret_modes, e = $5 in
        ...
        let vb =
          { pvb_pre_text = [];
            ...
            pvb_is_poly = false;
            pvb_pat = pat;
            pvb_legacy_modes = No_modes; pvb_modes = No_modes; pvb_params = params;
        ...
        in
        ($4, $3,
        Cfk_concrete ($1, vb)), No_attributes }
      ...
      { let constr =
        ...
        let vb =
          { pvb_pre_text = [];
            ...
            pvb_pat = pat;
            pvb_legacy_modes = No_modes; pvb_modes = No_modes; pvb_params = [];
            pvb_constraint = Some constr; pvb_ret_modes = No_modes;
            pvb_expr = Some $8;
            pvb_loc = make_loc $sloc;
            pvb_attributes = No_attributes (* FIXME: ? *);
            pvb_post_doc = None;
            pvb_tokens = Tokens.at $sloc; }
        in
        ($4, $3, Cfk_concrete ($1, vb)), No_attributes }
...
class_type:
  ...
  | mkcty(
      ...
      codomain = class_type
        { let domain =
            mk_arrow_arg ~loc:($symbolstartpos, $endpos(domain)) label No_modes domain No_modes
          in
...
class_signature:
  ...
  | cs=class_signature attribute
      { (* We are "patching" the signature to add an attribute, we must also
        ...
        relative to the attribute. *)
        let new_tokens, attributes =
          attributes_patching $sloc cs.pcty_tokens cs.pcty_attributes $2
        in
        { cs with pcty_tokens = new_tokens; pcty_attributes = attributes } }
    ...
    FUNCTION ext_attributes match_cases
      { let loc = make_loc $sloc in
        ...
        mkfunction [] empty_body_constraint body ~loc:$sloc
            ~ext_attrs:{ pea_ext = None; pea_attrs = No_attributes}
      }
    )
    { $1 }
;
...
simple_pattern_extend_modes_or_poly:
    simple_pattern { No_modes, $1, None, No_modes }
  ...
  | pat = pattern COLON cty_modes = poly_type_with_modes
      {
        ...
        No_modes, pat, Some cty, modes
      }
  | pat = pattern modes = at_mode_expr
      {
        No_modes, pat, None, modes
      }
  | pat = pattern COLON cty = strictly_poly_type
      {
        No_modes, pat, Some cty, No_modes
      }
;
...
fun_expr:
  ...
  | exp = fun_expr attribute
      { (* We are "patching" the expr to add an attribute, we must also patch
           ...
           relative to the attribute. *)
        let new_tokens, attributes =
          attributes_patching $sloc exp.pexp_tokens exp.pexp_attributes $2
        in
        { exp with
          pexp_tokens = new_tokens;
          pexp_attributes = attributes;
          pexp_loc = make_loc $sloc }
    }
...
%inline fun_expr_attrs:
  | LET MODULE ext_attributes module_name_modal(at_mode_expr, no_modes) module_binding_body IN seq_expr
      {
        ...
        in
        Pexp_letmodule(mb, $7), { pea_ext = None; pea_attrs = No_attributes } }
  ...
  | LET OPEN override_flag ext_attributes module_expr IN seq_expr
      { (* Similar remark as for LET MODULE above. *)
        ...
        in
        Pexp_let_open(od, $7), { pea_ext = None; pea_attrs = No_attributes } }
...
spliceable_expr:
  ...
  | LPAREN seq_expr opt_type_constraint_with_modes RPAREN
      { let (t, m) = $3 in
        mkexp_opt_type_constraint_with_modes ~ghost:true ~loc:$sloc ~modes:m $2 t }
  | mkrhs(val_longident)
...
%inline metaocaml_expr:
  | METAOCAML_ESCAPE e = simple_expr
    { mkexp ~loc:$sloc
        (pexp_extension ~loc:$sloc ~id:["metaocaml"; "escape"] e) }
  | METAOCAML_BRACKET_OPEN e = seq_expr METAOCAML_BRACKET_CLOSE
    { mkexp ~loc:$sloc
        (pexp_extension ~loc:$sloc ~id:["metaocaml"; "bracket"] e) }
;
...
let_binding_body_no_punning:
  ...
  | let_ident_with_modes COLON TYPE ntys = newtypes DOT cty = core_type modes1=no_modes EQUAL e = seq_expr
  ...
  | pattern_no_exn EQUAL seq_expr
      { { lbb_legacy_modes = No_modes; lbb_modes = No_modes; lbb_pat = $1; lbb_params = [];
          lbb_is_poly = false;
          lbb_constraint = None; lbb_ret_modes = No_modes; lbb_expr = Some $3 } }
...
;
let_binding_body:
  | poly_flag = poly_flag lbb = let_binding_body_no_punning
      { { lbb with lbb_is_poly = poly_flag } }
/* BEGIN AVOID */
  | poly_flag = poly_flag val_ident %prec below_HASH
      { { lbb_legacy_modes = No_modes; lbb_modes = No_modes;
          lbb_is_poly = poly_flag;
          lbb_pat = mkpatvar ~loc:$loc($2) $2;
          lbb_params = []; lbb_constraint = None; lbb_ret_modes = No_modes;
          lbb_expr = None } }
...
%inline pattern_(self):
  ...
  | pat=self attr=attribute
      { (* We are "patching" the pattern to add an attribute, we must also patch
           ...
           relative to the attribute. *)
        let new_tokens, attributes =
          attributes_patching $sloc pat.ppat_tokens pat.ppat_attributes attr
        in
        { pat with ppat_tokens = new_tokens; ppat_attributes = attributes }
      }
  | pattern_gen
      { $1 }
  | mkpat(
...
value_description:
  ...
  attrs = post_item_attributes
    { let loc = make_loc $sloc in
      let docs, sloc = symbol_docs $sloc in
      Val.mk id ty ~poly:poly_flag ~attrs ~modalities ~loc ~docs ~ext_attrs
        ~tokens:(Tokens.at sloc)
    }
;
...
jkind_annotation_gen(self) :
  self {
    { pjka_loc = make_loc $sloc; pjka_desc = $1; pjka_tokens = Tokens.at $sloc }
  }
;
...
label_declaration_semi:
    mutable_or_global_flag mkrhs(label) COLON poly_type_no_attr m1=optional_atat_modalities_expr attrs0=attributes
      ...
      { let info =
       ...
       in
       let mut, global = $1 in
       let tokens, attrs = merge_attrs (Tokens.at $sloc) attrs0 attrs1 in
       Type.field $2 $4 ~mut ~global ~modalities:m1 ~attrs
         ~tokens ~loc:(make_loc $sloc) ~info}
;
...
%inline type_extension(declaration):
  ...
  attrs = post_item_attributes
    { let loc = make_loc $sloc in
      let docs, sloc = symbol_docs $sloc in
      Te.mk tid cs ~params ~priv ~attrs ~docs ~loc ~ext_attrs
        ~tokens:(Tokens.at sloc)
    }
;
...
with_constraint:
  ...
  | KIND lid=mkrhs(label_longident) EQUAL
    jka=jkind_annotation_no_with_kinds
      { mkwc $sloc @@ Pwith_jkind (lid, jka) }
  | KIND lid=mkrhs(label_longident) COLONEQUAL
    jka=jkind_annotation_no_with_kinds
      { mkwc $sloc @@ Pwith_jkindsubst (lid, jka) }
;
with_type_binder:
    EQUAL          { Public }
...
%inline poly_type_with_optional_modes:
  ...
  | poly_type { $1, No_modes }
;

%inline strictly_poly_type_with_optional_modes:
  | strictly_poly_type { $1, No_modes }
...
core_type:
  ...
  | ty = core_type attribute
      { (* We are "patching" the type to add an attribute, we must also patch
           ...
           relative to the attribute. *)
        let new_tokens, attributes =
          attributes_patching $sloc ty.ptyp_tokens ty.ptyp_attributes $2
        in
        { ty with ptyp_tokens = new_tokens; ptyp_attributes = attributes } }
;

%inline core_type_with_optional_modes:
    core_type { $1, No_modes }
      ...
      codomain = strict_function_or_labeled_tuple_type
        { let arg_legacy_m, (typ, _), arg_modes = domain_with_modes in
          ...
          in
           Ptyp_arrow
             { domain ;
              codom_legacy_modes = No_modes;
              codom_type = codomain;
              codom_modes = No_modes }
             }
    )
    { $1 }
  | mktyp(
...
%inline param_type:
  ...
  | mktyp(
    LPAREN bound_vars = typevar_repr_list DOT inner_type = core_type RPAREN
      { let repr_loc = $startpos(bound_vars), $endpos(inner_type) in
        let repr = mktyp ~loc:repr_loc (Ptyp_repr (bound_vars, inner_type)) in
        Ptyp_parens repr }
    )
    { $1 }
  | mktyp(
    LPAREN LAYOUT bound_vars = newlayouts DOT inner_type = core_type RPAREN
      { let layout_loc = $startpos($2), $endpos(inner_type) in
        let layout = mktyp ~loc:layout_loc (Ptyp_newlayout (bound_vars, inner_type)) in
        Ptyp_parens layout }
    )
    { $1 }
  | ty = tuple_type
...
%inline field_semi:
  mkrhs(label) COLON poly_type_no_attr attributes SEMI attributes
    { let info =
        ...
        | None -> symbol_info $endpos
      in
      let tokens, attrs = merge_attrs (Tokens.at $sloc) $4 $6 in
      Of.tag ~loc:(make_loc $sloc) ~attrs ~info ~tokens $1 $3 }
;
...
value_constant:
  | INT               { let (n, m) = $1 in
                        mkconst ~loc:$sloc (Pconst_integer (None, n, m)) }
  | CHAR              { let (c, s) = $1 in
                        mkconst ~loc:$sloc (Pconst_char (c, s)) }
  | STRING            { let (s, strloc, d) = $1 in
                        mkconst ~loc:$sloc (Pconst_string (s, strloc, d)) }
  | FLOAT             { let (f, m) = $1 in
                        mkconst ~loc:$sloc (Pconst_float (None, f, m)) }
;
unboxed_constant:
  | HASH_INT          { mkconst ~loc:$sloc
                          (unboxed_int $sloc $sloc $1) }
  | HASH_FLOAT        { mkconst ~loc:$sloc (unboxed_float $1) }
  | HASH_CHAR         { let (c, s) = $1 in
                        mkconst ~loc:$sloc (Pconst_untagged_char (c, s)) }
;
constant:
    value_constant    { $1 }
  | unboxed_constant  { $1 }
...
signed_value_constant:
  ...
  | MINUS INT         { let (n, m) = $2 in
                        mkconst ~loc:$sloc (Pconst_integer(Some "-", n, m)) }
  | MINUS FLOAT       { let (f, m) = $2 in
                        mkconst ~loc:$sloc (Pconst_float(Some "-", f, m)) }
...
signed_constant:
  ...
  | MINUS HASH_INT        { mkconst ~loc:$sloc
                              (unboxed_int $sloc $loc($2) ~sign:Negative $2) }
  | MINUS HASH_FLOAT      { mkconst ~loc:$sloc
                              (unboxed_float ~sign:Negative $2) }
  | PLUS HASH_INT         { mkconst ~loc:$sloc
                              (unboxed_int $sloc $loc($2) ~sign:Positive $2) }
  | PLUS HASH_FLOAT       { mkconst ~loc:$sloc
                              (unboxed_float ~sign:Positive $2) }
;
...
constr_longident:
  ...
  | mod_longident DOT constr_extra_ident { mklid ~loc:$sloc @@
                                             ldot $1 $loc($1) $3 $loc($3) }
...
mk_longident(prefix,final):
   ...
   | prefix DOT final { mklid ~loc:$sloc @@ ldot $1 $loc($1) $3 $loc($3) }
;
val_longident:
...
%inline post_item_attributes:
  ...
  | post_item_attribute+
    { Attributes
        { attributes = $1; loc = make_loc $sloc; tokens = Tokens.at $sloc } }
;
%inline attributes:
  | /* empty */ { No_attributes }
  | attribute+
...
%inline ext_noattrs:
  | PERCENT attr_id
    { { pea_ext = Some $2; pea_attrs = No_attributes } }
;
