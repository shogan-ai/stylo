...
(* A docstring is "attached" if it has been inserted in the AST. This is used
   for generating unexpected docstring warnings. *)
type ds_attached =
...
type t =
  ...
  ; ds_loc : Location.t
  ; ds_id : int ref
  ; (* starts at [-1], is set when the ds gets attached. *)
    mutable ds_attached :
      ds_attached
