...
(* loc_ghost: Ghost expressions and patterns: expressions and patterns that do
   not appear explicitly in the source file they have the loc_ghost flag set to
   true. Then the profiler will not try to instrument them and the -annot option
   will not try to display their type.

   Every grammar rule that generates an element with a location must make at
   most one non-ghost element, the topmost one.

   How to tell whether your location must be ghost: A location corresponds to a
   range of characters in the source file. If the location contains a piece of
   code that is syntactically valid (according to the documentation), and
   corresponds to the AST node, then the location must be real; in all other
   cases, it must be ghost. *)
...
(** [merge locs] returns the location covering all locations from [locs]. It
    raises if [locs] is empty, and the result only makes sense if all the
    locations are from the same file. If [~ghost:false] is passed, the result
    location will only be ghost if one of the input locations was ghost. The
    default is [~ghost:true], which causes the result location to be ghost no
    matter what *)
