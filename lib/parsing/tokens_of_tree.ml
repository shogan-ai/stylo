...
module Error = struct
  ...
  let pp_context ppf { node_kind; pos } =
    Format.fprintf
      ppf
      "File %s, on %s at line %d column %d"
      pos.pos_fname
      node_kind
      pos.pos_lnum
  ...
  let pp_bounded_tokens ~limit ppf seq =
    ...
    fprintf
      ppf
      "@[<hov>%a%a@]"
      (pp_print_list ~pp_sep:pp_print_space Tokens.pp_elt)
      seq
      (fun ppf () -> (if did_truncate then fprintf else ifprintf) ppf "@ ...")
      ()
  ;;
  ...
  let pp_tokens_from_subtree ppf t =
    Format.fprintf
      ppf
      "@[<hov 4>%a@ (for %S subtree)@]"
      (pp_bounded_tokens ~limit:5)
      t.tokens
      t.origin
  ;;
  ...
  let pp ppf : t -> unit = function
    | `CST_tokens_mismatch (ctxt, node_tokens, subtokens) ->
      let expected_children =
        List.fold_left
          (fun nb t -> if Tokens.is_child t then nb + 1 else nb)
          0
          node_tokens
      in
      let limit = 10 in
      Format.fprintf
        ppf
        "@[<v>%a:@ @[<hov>Mismatch between@ this node's tokens skeleton@ and \
         the tokens retrieved@ from the subtrees.@]@;@[<hov 2>Current node \
         tokens skeleton@ (%d expected subtrees):@;%a@]@;@[<hov 2>Tokens \
         sequences@ for %d subtrees:@;@[<v>%a@]@]@."
        pp_context
        ctxt
        expected_children
        (pp_bounded_tokens ~limit)
        node_tokens
...
let combine_children node_kind ~loc top children =
  try [ { origin = node_kind; tokens = combine_children top children } ] with
  | Abort ->
...
let flatten_or_error
  : 'a. ('a -> tokens_from_subtree list) -> 'a -> (Tokens.seq, _) result
  =
  fun fn elt ->
  match fn elt with
...
let with_constraint = flatten_or_error tokenizer#with_constraint
