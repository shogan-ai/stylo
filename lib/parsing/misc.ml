...
(* This is a copy of the bits of the compiler's [utils/misc.ml] that are needed
   for parsing. *)
...
module Utf8_lexeme = struct
  ...
  let _ =
    List.iter
      (fun (upper, lower) ->
        let upper = Uchar.of_int upper
        and lower = Uchar.of_int lower in
  ...
  let _ =
    List.iter
      (fun (c1, n2, n) ->
        Hashtbl.add
          known_pairs
          (Uchar.of_char c1, Uchar.of_int n2)
          (Uchar.of_int n))
      [ ('A', 0x300, 0xc0)
      ; (* À *) ('A', 0x301, 0xc1)
      ; (* Á *) ('A', 0x302, 0xc2)
  ...
  let normalize_generic ~keep_ascii transform s =
    let rec norm check buf prev i =
      ...
      then (Buffer.add_utf_8_uchar buf (transform prev))
      else (
    ...
    then Ok s
      ...
      if !valid then Ok contents else Error contents
  ;;

  let normalize s = normalize_generic ~keep_ascii:true (fun u -> u) s
  ...
  let uchar_lowercase u =
    let c = Uchar.to_int u in
    if c < 0x80
    then if c >= 65 && c <= 90 then Uchar.of_int (c + 32) else u
    else
      match Hashtbl.find_opt known_chars u with
      | Some (Upper u') -> u'
      | _ -> u
  ...
  let uchar_uppercase u =
    let c = Uchar.to_int u in
    if c < 0x80
    then if c >= 97 && c <= 122 then Uchar.of_int (c - 32) else u
    else
      match Hashtbl.find_opt known_chars u with
      | Some (Lower u') -> u'
      | _ -> u
  ...
  let uchar_valid_in_identifier ~with_dot u =
    ...
    then
      ...
      || c = 39 (* single quote *)
      || (with_dot && c = 46) (* dot *)
    else Hashtbl.mem known_chars u
  ;;
  ...
  let validate_identifier ?(with_dot = false) s =
    let rec check i =
      ...
      then Valid
        ...
        let u = Uchar.utf_decode_uchar d in
        let i' = i + Uchar.utf_decode_length d in
        if not (uchar_valid_in_identifier ~with_dot u)
        then Invalid_character u
        else if i = 0 && uchar_not_identifier_start u
        then Invalid_beginning u
        else check i')
    in
    check 0
  ;;

  let is_valid_identifier s = validate_identifier s = Valid
