...
let empty_ext_attr = { pea_ext = None; pea_attrs = No_attributes }
...
module Docs = struct
  ...
  let text =
    List.filter_map (fun ds -> simplify_ds (Some ds) |> Option.map body_as_doc)
  ;;

  let pre_post docs =
    let pre_doc = simplify_ds docs.docs_pre in
    let post_doc = simplify_ds docs.docs_post in
    Option.map body_as_doc pre_doc, Option.map body_as_doc post_doc
  ;;
...
module Ext = struct
  let mk ?(docs = empty_docs) ext attrs =
    ...
    { te_pre_doc = pre; te_ext = ext; te_attrs = attrs; te_post_doc = post }
  ;;
end

module Const = struct
  let mk ?(loc = !default_loc) ~tokens d =
    { pconst_desc = d; pconst_loc = loc; pconst_tokens = tokens }
  ;;
  ...
  let int ?loc ?suffix ~tokens i =
    integer ?loc ?suffix ~tokens (Int.to_string i)
  ;;
...
module Typ = struct
  ...
  let unboxed_tuple ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Ptyp_unboxed_tuple a)
  ;;

  let constr ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ptyp_constr (a, b))
  ;;

  let object_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ptyp_object (a, b))
  ;;

  let class_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ptyp_class (a, b))
  ;;

  let alias ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Ptyp_alias (a, b, c))
  ;;

  let variant ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Ptyp_variant (a, b, c))
  ;;

  let poly ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Ptyp_poly (a, b))
  ;;
  (* let package ?loc ?attrs ~tokens a b = let pkg =
     [{ ppt_ext_attr = None; ppt_name = a; ppt_eqs = b }] in mk ?loc ?attrs
     ~tokens (Ptyp_package pkg) *)

  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Ptyp_extension a)
  ;;

  let open_ ?loc ?attrs ~tokens mod_ident t =
    mk ?loc ?attrs ~tokens (Ptyp_open (mod_ident, t))
  ;;
end
...
module Pat = struct
  ...
  let interval ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_interval (a, b))
  ;;

  let tuple ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Ppat_tuple (a, b))

  let unboxed_tuple ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_unboxed_tuple (a, b))
  ;;

  let construct ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_construct (a, b))
  ;;

  let variant ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_variant (a, b))
  ;;

  let record ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_record (a, b))
  ;;
  ...
  let constraint_ ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Ppat_constraint (a, b, c))
  ;;

  let type_ ?loc ?attrs ~tokens a = mk ?loc ?attrs ~tokens (Ppat_type a)
  let lazy_ ?loc ?attrs ~tokens a = mk ?loc ?attrs ~tokens (Ppat_lazy a)

  let unpack ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Ppat_unpack (a, b))
  ;;

  let open_ ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Ppat_open (a, b))

  let exception_ ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Ppat_exception a)
  ;;

  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Ppat_extension a)
  ;;
end
...
module Exp = struct
  ...
  let let_ ?loc ?attrs ~tokens a b c d =
    mk ?loc ?attrs ~tokens (Pexp_let (a, b, c, d))
  ;;

  let function_ ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pexp_function (a, b, c))
  ;;

  let apply ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Pexp_apply (a, b))

  let match_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_match (a, b))
  ;;
  ...
  let unboxed_tuple ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pexp_unboxed_tuple a)
  ;;

  let construct ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_construct (a, b))
  ;;

  let variant ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_variant (a, b))
  ;;

  let record ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_record (b, a))
  ;;
  ...
  let unboxed_field ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_unboxed_field (a, b))
  ;;

  let setfield ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pexp_setfield (a, b, c))
  ;;

  let array ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Pexp_array (a, b))

  let ifthenelse ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pexp_ifthenelse (a, b, c))
  ;;

  let sequence ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_sequence (a, b))
  ;;

  let while_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_while (a, b))
  ;;

  let for_ ?loc ?attrs ~tokens a b c d e =
    mk ?loc ?attrs ~tokens (Pexp_for (a, b, c, d, e))
  ;;

  let constraint_ ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pexp_constraint (a, b, c))
  ;;

  let coerce ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pexp_coerce (a, b, c))
  ;;
  ...
  let setinstvar ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_setvar (a, b))
  ;;

  let override ?loc ?attrs ~tokens a = mk ?loc ?attrs ~tokens (Pexp_override a)

  let letmodule ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_letmodule (a, b))
  ;;

  let letexception ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_letexception (a, b))
  ;;
  ...
  let pack ?loc ?attrs ~tokens ?pkg_type a =
    mk ?loc ?attrs ~tokens (Pexp_pack (a, pkg_type))
  ;;

  let open_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_dot_open (a, b))
  ;;
  ...
  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pexp_extension a)
  ;;

  let unreachable ?loc ?attrs ~tokens () =
    mk ?loc ?attrs ~tokens Pexp_unreachable
  ;;

  let stack ?loc ?attrs ~tokens e = mk ?loc ?attrs ~tokens (Pexp_stack e)

  let comprehension ?loc ?attrs ~tokens e =
    mk ?loc ?attrs ~tokens (Pexp_comprehension e)
  ;;

  let overwrite ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pexp_overwrite (a, b))
  ;;
  ...
  let binding_op op vb loc = { pbop_op = op; pbop_binding = vb; pbop_loc = loc }
end
...
module Mty = struct
  ...
  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pmty_extension a)
  ;;

  let strengthen ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pmty_strengthen (a, b))
  ;;
end
...
module Mod = struct
  ...
  let apply ?loc ?attrs ~tokens m1 m2 =
    mk ?loc ?attrs ~tokens (Pmod_apply (m1, m2))
  ;;

  let apply_unit ?loc ?attrs ~tokens m1 =
    mk ?loc ?attrs ~tokens (Pmod_apply_unit m1)
  ;;
  ...
  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pmod_extension a)
  ;;
...
module Sig = struct
  ...
  (* let value ?loc a = mk ?loc (Psig_value a) let type_ ?loc rec_flag a = mk
     ?loc (Psig_type (rec_flag, a)) let type_subst ?loc a = mk ?loc
     (Psig_typesubst a) let type_extension ?loc a = mk ?loc (Psig_typext a) let
     exception_ ?loc a = mk ?loc (Psig_exception a) let module_ ?loc a = mk ?loc
     (Psig_module a) let mod_subst ?loc a = mk ?loc (Psig_modsubst a) let
     rec_module ?loc a = mk ?loc (Psig_recmodule a) let modtype ?loc a = mk ?loc
     (Psig_modtype a) let modtype_subst ?loc a = mk ?loc (Psig_modtypesubst a)
     let open_ ?loc a = mk ?loc (Psig_open a) let include_ ?loc ?(modalities =
     []) a = mk ?loc (Psig_include (a, modalities)) let class_ ?loc a = mk ?loc
     (Psig_class a) let class_type ?loc a = mk ?loc (Psig_class_type a) let
     extension ?loc ?(attrs = No_attributes) a = mk ?loc (Psig_extension (a,
     attrs)) let kind_abbrev ?loc a b = mk ?loc (Psig_kind_abbrev (a, b)) let
     attribute ?loc ~tokens a = mk ?loc ~tokens (Psig_attribute a) *)
  let text txt =
    List.map
      (fun ds ->
...
module Str = struct
  ...
  let eval ?loc ?(attrs = No_attributes) ~tokens a =
    mk ?loc ~tokens (Pstr_eval (a, attrs))
  ;;

  (* let value ?loc a b = mk ?loc (Pstr_value (a, b)) let primitive ?loc a = mk
     ?loc (Pstr_primitive a) let type_ ?loc rec_flag a = mk ?loc (Pstr_type
     (rec_flag, a)) let type_extension ?loc a = mk ?loc (Pstr_typext a) let
     exception_ ?loc a = mk ?loc (Pstr_exception a) let module_ ?loc a = mk ?loc
     (Pstr_module a) let rec_module ?loc a = mk ?loc (Pstr_recmodule a) let
     modtype ?loc a = mk ?loc (Pstr_modtype a) let open_ ?loc a = mk ?loc
     (Pstr_open a) let class_ ?loc a = mk ?loc (Pstr_class a) let class_type
     ?loc a = mk ?loc (Pstr_class_type a) let include_ ?loc a = mk ?loc
     (Pstr_include a) let extension ?loc ?(attrs = No_attributes) a = mk ?loc
     (Pstr_extension (a, attrs)) let kind_abbrev ?loc a b = mk ?loc
     (Pstr_kind_abbrev (a, b)) let attribute ?loc ~tokens a = mk ?loc ~tokens
     (Pstr_attribute a) *)
  let text txt =
    List.map
      (fun ds ->
...
module Cl = struct
  ...
  let constr ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pcl_constr (a, b))
  ;;
  ...
  let let_ ?loc ?attrs ~tokens a b c =
    mk ?loc ?attrs ~tokens (Pcl_let (a, b, c))
  ;;

  let constraint_ ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pcl_constraint (a, b))
  ;;
...
module Cty = struct
  ...
  let constr ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pcty_constr (a, b))
  ;;

  let signature ?loc ?attrs ~tokens a b =
    mk ?loc ?attrs ~tokens (Pcty_signature (a, b))
  ;;

  let arrow ?loc ?attrs ~tokens a b = mk ?loc ?attrs ~tokens (Pcty_arrow (a, b))

  let extension ?loc ?attrs ~tokens a =
    mk ?loc ?attrs ~tokens (Pcty_extension a)
  ;;
...
module Ctf = struct
  ...
  (* let inherit_ ?loc ?attrs a = mk ?loc ?attrs (Pctf_inherit a) let val_ ?loc
     ?attrs a b c d = mk ?loc ?attrs (Pctf_val (a, b, c, d)) let method_ ?loc
     ?attrs a b c d = mk ?loc ?attrs (Pctf_method (a, b, c, d)) let constraint_
     ?loc ?attrs a b = mk ?loc ?attrs (Pctf_constraint (a, b)) let extension
     ?loc ?attrs a = mk ?loc ?attrs (Pctf_extension a) let attribute ?loc a = mk
     ?loc (Pctf_attribute a) *)
  let text txt =
    List.map
      (fun ds ->
        let loc = docstring_loc ds in
...
module Cf = struct
  ...
  (* let inherit_ ?loc ?attrs a b c = mk ?loc ?attrs (Pcf_inherit (a, b, c)) let
     val_ ?loc ?attrs a b c = mk ?loc ?attrs (Pcf_val (a, b, c)) let method_
     ?loc ?attrs a b c = mk ?loc ?attrs (Pcf_method (a, b, c)) let constraint_
     ?loc ?attrs a b = mk ?loc ?attrs (Pcf_constraint (a, b)) let initializer_
     ?loc ?attrs a = mk ?loc ?attrs (Pcf_initializer a) let extension ?loc
     ?attrs a = mk ?loc ?attrs (Pcf_extension a) let attribute ?loc ~tokens a =
     mk ?loc ~tokens (Pcf_attribute a) *)
  let text txt =
    List.map
      (fun ds ->
        let loc = docstring_loc ds in
...
module Rf = struct
  ...
  let inherit_ ?loc ~tokens ty = mk ?loc ~tokens (Rinherit ty)
end
...
module Of = struct
  ...
  let inherit_ ?loc ~tokens ty = mk ?loc ~tokens (Oinherit ty)
end
