...
and constant_desc =
  | Pconst_integer of string option * string * char option
    (** Integer constants such as [3] [-3] [-3l] [3L] [3n].
        ...
        Suffixes [[g-z][G-Z]] are accepted by the parser. Suffixes except ['l'],
        ['L'] and ['n'] are rejected by the typechecker *)
  | Pconst_unboxed_integer of string option * string * char
    (** Integer constants such as [#3] [#3l] [#3L] [#3n].

        A suffix [[g-z][G-Z]] is required by the parser. Suffixes except ['l'],
        ['L'] and ['n'] are rejected by the typechecker *)
  ...
  | Pconst_float of string option * string * char option
    (** Float constant such as [3.4], [2e5] or [1.4e-4].
        ...
        Suffixes [g-z][G-Z] are accepted by the parser. Suffixes except ['s']
...
and core_type_desc =
  ...
  | Ptyp_unboxed_tuple of (string option * core_type) list
    (** Unboxed tuple types:
        [Ptyp_unboxed_tuple([(Some l1,P1);...;(Some l2,Pn)]] represents a
        product type [#(l1:T1 * ... * l2:Tn)], and the labels are optional.
  ...
  | Ptyp_variant of row_field list * closed_flag * label list option
    (** [Ptyp_variant([`A;`B], flag, labels)] represents:
        - [[ `A|`B ]] when [flag] is {{!Asttypes.closed_flag.Closed}[Closed]},
          and [labels] is [None],
        - [[> `A|`B ]] when [flag] is {{!Asttypes.closed_flag.Open}[Open]}, and
          [labels] is [None],
        - [[< `A|`B ]] when [flag] is {{!Asttypes.closed_flag.Closed}[Closed]},
          and [labels] is [Some []],
  ...
  | Ptyp_poly of bound_ty_var list * core_type
    (** ['a1 ... 'an. T] [('a1 : k1) ... ('an : kn). T]
        ...
        - Under {{!class_field_kind.Cfk_virtual}[Cfk_virtual]} for methods (not
          values).
...
and pattern_desc =
  ...
  | Ppat_unboxed_tuple of pattern argument list * closed_flag
    (** Unboxed tuple patterns: [#(l1:P1, ..., ln:Pn)] is
        [([(Some l1,P1);...;(Some l2,Pn)], Closed)], and the labels are
        optional. An [Open] pattern ends in [..].
  ...
  | Ppat_record_unboxed_product of pattern record_field list * closed_flag
    (** [Ppat_record_unboxed_product([(l1, P1) ; ... ; (ln, Pn)], flag)]
        represents:
        - [#{ l1=P1; ...; ln=Pn }] when [flag] is
          {{!Asttypes.closed_flag.Closed}[Closed]}
        - [#{ l1=P1; ...; ln=Pn; _}] when [flag] is
...
and expression_desc =
  | Pexp_ident of Longident.t loc (** Identifiers such as [x] and [M.x] *)
  ...
  | Pexp_function of function_param list * function_constraint * function_body
    (** [Pexp_function ([P1; ...; Pn], C, body)] represents any construct
        ...
        - [fun P1 ... Pn -> function p1 -> e1 | ... | pm -> em] when
          [body = Pfunction_cases [ p1 -> e1; ...; pm -> em ]] [C] represents a
          type constraint or coercion placed immediately before the arrow, e.g.
  ...
  | Pexp_record_unboxed_product of
    ...
    (** [Pexp_record_unboxed_product(exp0, [(l1,P1) ; ... ; (ln,Pn)])]
        represents
        - [#{ l1=P1; ...; ln=Pn }] when [exp0] is [None]
        - [#{ E0 with l1=P1; ...; ln=Pn }] when [exp0] is [Some E0]
  ...
  | Pexp_idx of block_access * unboxed_access list
    (** [(BA1 UA1 UA2 ...)] e.g. [(.foo.#bar.#baz)] Above, BA1=.foo, UA1=.#bar,
        and UA2=#.baz *)
  ...
  | Pexp_dot_open of Longident.t loc * expression (** - [M.(E)] *)
...
and function_param_desc =
  | Pparam_val of pattern argument
    (** [Pparam_val (lbl, exp0, P)] represents the parameter:
        - [P] when [lbl] is {{!Asttypes.arg_label.Nolabel}[Nolabel]} and [exp0]
          is [None]
        - [~l:P] when [lbl] is {{!Asttypes.arg_label.Labelled}[Labelled l]} and
          [exp0] is [None]
        - [?l:P] when [lbl] is {{!Asttypes.arg_label.Optional}[Optional l]} and
          [exp0] is [None]
        - [?l:(P = E0)] when [lbl] is
  ...
  | Pparam_newtypes of bound_ty_var list
    (** [Pparam_newtype x] represents the parameter [(type x)]. [x] carries the
        location of the identifier, whereas the [pparam_loc] on the enclosing
        [function_param] node is the location of the [(type x)] as a whole.
...
and function_constraint =
  { ret_mode_annotations : modes
       (** The mode annotation placed on a function's body, e.g.
           [let f x : int -> int @@ local = ...]. This field constrains the mode
           of function's body. *)
  ; ret_type_constraint : type_constraint option
       (** The type constraint placed on a function's body. *)
  }
and block_access =
  | Baccess_field of Longident.t loc (** [.foo] *)
  | Baccess_block of mutable_flag * expression
    (** Access using another block index: [.idx_imm(E)], [.idx_mut(E)] (usually
        followed by unboxed accesses, to deepen the index). *)

and unboxed_access =
  | Uaccess_unboxed_field of Longident.t loc (** [.#foo] *)
...
and comprehension_iterator =
  | Pcomp_range of
    ...
    (** "= START to STOP" (direction = Upto) "= START downto STOP" (direction =
        Downto) *)
...
and extension_constructor_kind =
  | Pext_decl of bound_ty_var list * constructor_arguments * core_type option
...
and class_type_field_desc =
  ...
  | Pctf_val of
      attributes
      *
      (string loc * mutable_flag * virtual_flag * core_type)
    (** [val x: T] *)
  | Pctf_method of
      attributes
      *
      (string loc * private_flag * virtual_flag * core_type)
    (** [method x: T]

        Note: [T] can be a {{!core_type_desc.Ptyp_poly}[Ptyp_poly]}. *)
  | Pctf_constraint of attributes * (core_type * core_type)
    (** [constraint T1 = T2] *)
...
and class_expr_desc =
  ...
  | Pcl_fun of pattern argument list * class_expr
    (** [Pcl_fun(lbl, exp0, P, CE)] represents:
        - [fun P -> CE] when [lbl] is {{!arg_label.Nolabel}[Nolabel]} and [exp0]
          is [None],
        - [fun ~l:P -> CE] when [lbl] is {{!arg_label.Labelled}[Labelled l]} and
          [exp0] is [None],
        - [fun ?l:P -> CE] when [lbl] is {{!arg_label.Optional}[Optional l]} and
          [exp0] is [None],
        - [fun ?l:(P = E0) -> CE] when [lbl] is
          {{!arg_label.Optional}[Optional l]} and [exp0] is [Some E0]. *)
...
and class_field_desc =
  ...
  | Pcf_constraint of attributes * (core_type * core_type)
    (** [constraint T1 = T2] *)
...
and with_constraint_desc =
  | Pwith_type of
    ...
    (** [with type X.t = ...]
        ...
        Note: the last component of the Longident.t must match the name of the
        type_declaration. *)
  | Pwith_module of Longident.t loc * Longident.t loc
    (** [with module X.Y = Z] *)
  | Pwith_modtype of Longident.t loc * module_type
    (** [with module type X.Y = Z] *)
  | Pwith_jkind of Longident.t loc * jkind_annotation
    (** [with kind_ X.k = ...] *)
  ...
  | Pwith_modsubst of Longident.t loc * Longident.t loc
    (** [with module X.Y := Z] *)
  | Pwith_jkindsubst of Longident.t loc * jkind_annotation
    (** [with kind_ X.k := ...] *)
...
and jkind_annotation_desc =
  ...
  | Pjk_operator of jkind_annotation * string loc list
    (** [Pjk_operator(K, [SA1; ...; SAn])] represents the layout [K SA1 ... SAn]
        where [K] is an arbitrary kind and each [SAi] is a scannable axis
        annotation (like [non_pointer]). *)
  (* CR layouts v2.8: [mod] can have only layouts on the left, not full kind
     annotations. We may want to narrow this type some. *)
