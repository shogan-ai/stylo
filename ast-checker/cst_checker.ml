...
let normalize_cmt_spaces doc =
  String.split_on_char ' ' doc |> List.filter (( <> ) "") |> String.concat " "
;;
...
(* method! visit_expression env exp = let [{pexp_desc; pexp_attributes; _}] = exp in match
   pexp_desc with (* convert [(c1; c2); c3] to [c1; (c2; c3)] *) | Pexp_sequence
   ([{pexp_desc= Pexp_sequence (e1, e2); pexp_attributes= []; _}], e3) -> (* FIXME: what
   about ext_attrs?! *) self#visit_expression env (Exp.sequence e1 (Exp.sequence
   ~attrs:pexp_attributes e2 e3)) | _ -> super#visit_expression env exp

   method! visit_pattern env pat = let
   [{ppat_desc; ppat_loc= loc1; ppat_attributes= attrs1; _}] = pat in (* normalize nested
   or patterns *) match ppat_desc with | Ppat_or ( pat1 ,
...
*)
(* let report_error lex exn = match Location.error_of_exn exn with | None -> let loc =
   [{ Location.loc_start = lex.Lexing.lex_start_p ; loc_end = lex.Lexing.lex_curr_p ; loc_ghost = true }]
   in Format.eprintf "%a@ Unexpected exn: %s@." Location.print_loc loc (Printexc.to_string
   exn); exit 1 | Some `Already_displayed -> assert false | Some `Ok report ->
   Format.eprintf "%a@." Location.print_report report; exit 1
