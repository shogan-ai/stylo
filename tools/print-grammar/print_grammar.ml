...
let () = (* This will collect positional (anonymous) arguments *)
  ...
  let spec =
    [ ( "--entrypoint"
      , Arg.String (fun s -> entrypoints := s :: !entrypoints)
      , "<symbol> Entrypoint to process" )
    ; ( "-e"
      , Arg.String (fun s -> entrypoints := s :: !entrypoints)
      , "<symbol> Entrypoint to process" )
    ; ( "-i"
      , Arg.String (fun s -> inlined_terms := s :: !inlined_terms)
      , "<term> Term to inline" )
    ; ( "--inline"
      , Arg.String (fun s -> inlined_terms := s :: !inlined_terms)
      , "<term> Term to inline" )
  ...
  let usage_msg =
    "print_grammar [--inline term] [--entrypoint symbol] <grammar.cmly>]"
  in
  Arg.parse
    spec
    (fun s ->
      ...
      then grammar := s
        ...
        raise
          (Arg.Bad
             (Printf.sprintf "input specified twice (%S and %S)" s !grammar)))
    usage_msg;
  if !grammar = ""
  then (
...
let errorf fmt = Printf.kfprintf (fun _ -> exit 1) stderr fmt

(* Reverse engineer anonymous producers. Minor bug in Menhir: the grammar is
   serialized post lambda-lifting. It shouldn't. *)
...
let () =
  List.iter
    (fun (name, def) ->
      if String.starts_with ~prefix:"__anonymous_" name
      then Hashtbl.add anonymous name def)
...
let resolve_anonymous param =
  ...
  match desc with
  | Var var ->
    (match Hashtbl.find_opt anonymous var with
     | None -> desc
     | Some rule ->
       assert (valid_rule rule []);
       Anonymous (Rule.branches rule))
  | App (name, args) ->
    (match Hashtbl.find_opt anonymous name with
     | None -> desc
     | Some rule ->
...
type term =
  | App of string * bool (* should we inline this term? *) * term list
...
let () =
  List.iter
    (fun (name, def) ->
      if (match Rule.parameters def with
          | [ var ] ->
            (match Rule.branches def with
             | [ branch ] ->
               (match Branch.producers branch with
                | [ prod ] ->
                  (match Parameter.desc (Producer.symbol prod) with
                   | Var var' -> (var = var')
                   | _ -> false)
                | _ -> false)
             | _ -> false)
          | _ -> false)
      then inlined_terms := name :: !inlined_terms)
    (Syntax.rules before_expansion)
;;
...
let need_inlining =
  ...
  let parse_spec term =
    if String.exists is_meta term
    then
      let position = ref 0 in
      let tokens = ref [] in
      let flush pos =
        if !position <> pos
        then tokens := String.sub term !position (pos - !position) :: !tokens;
        position := pos + 1
      in
      for i = 0 to String.length term - 1 do
        if is_ws term.[i]
        then flush i
        else if is_operator term.[i]
        then (
          flush i;
          tokens := String.make 1 term.[i] :: !tokens)
      done;
      let malformed () = errorf "inlining term: %S is malformed\n" term in
      if !position <> String.length term then malformed ();
  ...
  List.iter
    (fun spec ->
      List.iter
        (fun term ->
          match term with
          | App (name, _, []) -> names := StringSet.add name !names
...
let rec lift_parameter param =
  let mk_app head args =
    ...
    if need_inlining result then App (head, true, args) else result
  in
  match resolve_anonymous param with
  | App (head, args) -> mk_app head (List.map lift_parameter args)
  | Var name -> mk_app name []
  | Anonymous branches -> Anonymous (List.map lift_branch branches)
...
let rules =
  ...
  List.iter
    (fun (name, def) -> Hashtbl.add table name (lift_rule def))
    (Syntax.rules before_expansion);
  table
;;
...
let rec traverse_seq (f : term -> term list list) : term list -> term list list
  = function
  | [] -> [ [] ]
  | x :: xs ->
    let fx = f x
    and fxs = traverse_seq f xs in
    List.concat_map (fun x' -> List.map (fun xs' -> x' @ xs') fxs) fx
;;

let traverse_branches f branches = List.concat_map (traverse_seq f) branches

let subst env =
  let rec aux = function
    | Anonymous branches -> Anonymous (List.map (List.map aux) branches)
...
let rec inline import blocked abstract = function
  | App (head, do_inline, args) as term
    ...
    (match Hashtbl.find_opt rules head with
     | None -> assert false
  ...
  | Anonymous branches -> inline_branches import blocked abstract branches
  | term -> [ [ inline_args import blocked abstract term ] ]

and inline_args import blocked abstract = function
  | Anonymous [ [ term ] ] -> inline_args import blocked abstract term
  | Anonymous branches ->
  ...
  | App (head, do_inline, args) ->
    if not (List.mem head abstract) then import head;
...
let rec import_rule =
  ...
  | "error" -> ()
  | name ->
    if not (Hashtbl.mem imported name)
    then (
      Hashtbl.add imported name ();
      match Hashtbl.find_opt rules name with
      | Some (parameters, branches) ->
        let branches =
          inline_branches import_rule TermSet.empty parameters branches
        in
        definitions := (name, parameters, branches) :: !definitions
      | None ->
        if not (List.mem_assoc name tokens)
        then errorf "Unknown rule %s.\n" name)
;;
...
let rec print_parameter = function
  ...
  | Anonymous [ branch ] -> print_inline_branch 0 branch
  | Anonymous branches ->
    print_char '(';
...
let () =
  List.iter
    print_rule
    (List.sort (fun (a, _, _) (b, _, _) -> String.compare a b) !definitions)
;;
